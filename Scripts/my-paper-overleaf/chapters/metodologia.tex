
Nuestra metodología está organizada por etapas alineadas con los objetivos específicos del proyecto (Capítulo~\ref{sec:objetivos}). El enfoque general consiste en integrar datos, limpiarlos, analizarlos y construir modelos para predecir los indicadores $SAIDI$ y $SAIFI$ de la empresa de distribución eléctrica en Norte de Santander. La correspondencia entre etapas y objetivos se resume al final de este capítulo en la Tabla~\ref{tab:etapas_objetivos}.


Se decidió utilizar CRISP-DM (Cross-Industry Standard Process for Data Mining) como marco de referencia por ser un estándar reconocido en la industria para proyectos de ciencia de datos~\textcite{ref45}, el cual fue adaptado al contexto específico del sector eléctrico regulado. Las fases de CRISP-DM se implementaron de la siguiente manera: el entendimiento del negocio (Business Understanding) se desarrolla en las Secciones~\ref{sec:metodología-etapa1} a~\ref{sec:metodología-etapa4}. Las fases de entendimiento y preparación de datos (Data Understanding y Data Preparation) corresponden a las Etapas~1 a~4, en Secciones~\ref{sec:metodología-etapa1}, \ref{sec:metodología-etapa2}, \ref{sec:metodología-etapa3} y~\ref{sec:metodología-etapa4}, en las que se integran, depuran y exploran los datos y se construyen las variables analíticas. La fase de modelado y evaluación (Modeling y Evaluation) se desarrolla en la Etapa~5, en la Sección~\ref{sec:metodología-etapa5}, donde se construyen y validan los modelos predictivos. Finalmente, el despliegue (Deployment) corresponde a la Etapa~6 en la Sección~\ref{sec:metodología-etapa6}, en la cual se consolidan los resultados, se documenta el proceso y se entrega código reproducible.

Esta adaptación de CRISP-DM considera los requisitos específicos del regulador (CREG, SSPD), las métricas propias del sector eléctrico ($SAIDI$, $SAIFI$) y la necesidad de que los modelos sean interpretables para su uso en la toma de decisiones.




\section{Etapa 1 – Extracción de datos}
\label{sec:metodología-etapa1}

Esta etapa comprende la recolección, integración y estandarización de las fuentes de datos internas (SCADA, OMS, GIS, CIS, registros de fallas y mantenimiento) y externas (NASA, CREG, SSPD). Incluye la creación de un repositorio analítico unificado y la elaboración de un catálogo de datos que documenta metadatos, alcances, limitaciones y métricas de integridad.

\subsubsection{Fuentes de informacion}
A partir del año $2023$, la Electrificadora de Norte de Santander realizó una migración de su sistema de gestión de interrupciones. Como resultado, la recopilación de información de interrupciones eléctricas se llevó a cabo a partir de dos bases de datos provenientes de sistemas distintos:

\begin{itemize}
\item \textbf{OMS Energy}: sistema utilizado hasta antes de la migración, del cual se extrae la información histórica de interrupciones previas a $2023$.
        
\item \textbf{OMS SP7 de Siemens}: nuevo sistema implementado desde $2023$, del cual se obtiene la información de interrupciones a partir de dicha fecha.
\end{itemize}

Esta dualidad de fuentes implica diferencias estructurales en los esquemas de datos (tales como nombres de campos, codificaciones y niveles de granularidad), las cuales fueron consideradas durante el proceso de integración y homologación de la información.


\subsection{Sistemas y fuentes consultadas}
Se integran datos de cinco fuentes de información principales:

\begin{itemize}
        \item \textbf{BRAE}: base regulatoria e histórica de eventos.
        
        \item \textit{OMS SPARD} (sistema histórico): fuente de elemento causante antes de la migración.
        
        \item \textit{OMS SP7} (sistema actual): fuente de elemento causante en archivos CSV mensuales.
        
        \item \textbf{GTECH (GIS)}: inventario y georreferenciación de activos eléctricos.
        
        \item \textbf{NASA POWER (API)}: serie diaria de variables climatológicas por coordenada/cuadrante.
\end{itemize}


\subsection{Extracción base y depuración de eventos de interrupción (BRAE)}
\label{sec:extraccion_brae}
La construcción de la base de eventos de interrupción se realizó a partir de la tabla transaccional \texttt{BRAE.QA\_TFDDREGISTRO}, perteneciente a la Base Regulatoria de Activos Eléctricos (BRAE), la cual contiene el registro histórico de eventos de desconexión del sistema eléctrico. A partir de esta fuente, se ejecutó una consulta SQL orientada a depurar inconsistencias, restringir el periodo de análisis y estandarizar la estructura del identificador de evento.

En primer lugar, se seleccionaron únicamente los campos relevantes para el análisis: el identificador del evento (\texttt{FDD\_CODIGOEVENTO}), la fecha de inicio de la desconexión (\texttt{FDD\_FINICIAL}) y la fecha de reconexión (\texttt{FDD\_FFINAL}). Adicionalmente, se incluyó un campo vacío para \texttt{ELEMENTO\_FALLADO}, con el fin de mantener compatibilidad estructural con otras fuentes que sí contienen este nivel de desagregación.

Posteriormente, se aplicaron criterios de depuración y filtrado. En primer lugar, se restringió la muestra al periodo comprendido entre los años 2021 y 2025, garantizando consistencia temporal del análisis. En segundo lugar, se excluyeron aquellos registros marcados como no relevantes mediante la variable \texttt{FDD\_EXCLUSION}, conservando únicamente los eventos clasificados como \texttt{NO EXCLUIDA}. En tercer lugar, se eliminaron eventos asociados a causas no válidas según la regulación (\texttt{FDD\_CAUSA\_SSPD NOT IN (1)}), así como aquellos registros sin fecha de finalización (\texttt{FDD\_FFINAL IS NOT NULL}), los cuales no permiten calcular la duración del evento.

    Finalmente, se utilizó la instrucción \texttt{DISTINCT} para garantizar la eliminación de duplicados en el identificador de evento, obteniéndose una base depurada y consistente a nivel de evento de interrupción. Como resultado de este proceso de extracción y limpieza, se obtuvo un total de 95{.}280 registros válidos de eventos de desconexión para el periodo de análisis. La Tabla~\ref{tab:muestra_eventos_brae} presenta una muestra de la estructura resultante. El código SQL correspondiente se presenta en el Listado~\ref{lst:sql_brae} (Apéndice~\ref{app:codigo}).

    
    \begin{table}[!htpb]
\centering
\small
\caption{Muestra de eventos depurados de interrupción (BRAE)}
\label{tab:muestra_eventos_brae}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{1.8cm} >{\raggedright\arraybackslash}p{3.7cm} >{\raggedright\arraybackslash}p{3.7cm} >{\raggedright\arraybackslash}p{2.6cm}}
\toprule
    \textbf{Id evento} & \textbf{Fecha desconexión} & \textbf{Fecha conexión} & \textbf{Elemento fallado} \\
\midrule
801851 & 14/03/21 06:59:00.000 & 14/03/21 09:30:00.000 & (null) \\
787583 & 04/01/21 08:50:00.000 & 04/01/21 17:15:00.000 & (null) \\
787496 & 03/01/21 14:18:14.296 & 03/01/21 14:40:42.699 & (null) \\
787232 & 01/01/21 10:29:33.936 & 01/01/21 13:30:00.000 & (null) \\
787455 & 03/01/21 10:34:23.000 & 03/01/21 13:30:00.000 & (null) \\
787376 & 02/01/21 12:09:22.000 & 02/01/21 18:20:00.000 & (null) \\
789756 & 16/01/21 08:50:00.000 & 16/01/21 15:50:00.000 & (null) \\
788677 & 10/01/21 08:37:45.000 & 10/01/21 10:10:00.000 & (null) \\
788932 & 12/01/21 06:47:47.000 & 12/01/21 08:30:00.000 & (null) \\
787922 & 06/01/21 08:42:00.000 & 06/01/21 15:28:00.000 & (null) \\
\bottomrule
\end{tabularx}
\end{table}


\subsection{Extracción de afectación por evento}
\label{sec:extraccion_afectacion}
Con el fin de complementar la base de eventos de interrupción, se construyó una tabla de afectación asociada a cada evento, incorporando información sobre el número de elementos afectados y la cantidad de usuarios impactados. Este proceso permite pasar de una caracterización puramente temporal del evento a una medición de su severidad en términos de impacto en la red.

La extracción se realizó a partir de la tabla \texttt{BRAE.QA\_TFDDREGISTRO}, la cual se utilizó como fuente principal de eventos, y se complementó mediante una unión (LEFT JOIN) con la base \texttt{QA\_TTC1}, que contiene información asociada a la conexión de transformadores y el número de usuarios por conexión.

En particular, la variable \texttt{CANTIDAD\_USUARIOS} se construyó a partir del conteo de usuarios asociados a cada transformador (\texttt{TC1\_CODCONEX}) agrupados por periodo, mientras que la variable de afectación por evento se obtuvo mediante la agregación de estos usuarios a nivel de evento de interrupción.

Posteriormente, se aplicaron los mismos criterios de depuración utilizados en la etapa anterior, restringiendo el análisis al periodo 2021–2025, excluyendo eventos marcados como no relevantes (\texttt{FDD\_EXCLUSION = 'NO EXCLUIDA'}), eliminando causas no válidas según la clasificación regulatoria (\texttt{FDD\_CAUSA\_SSPD NOT IN (1)}) y conservando únicamente registros con fecha de finalización válida (\texttt{FDD\_FFINAL IS NOT NULL}).

    Finalmente, los datos se agregaron a nivel de evento (\texttt{FDD\_CODIGOEVENTO}) utilizando la función \texttt{GROUP BY}, obteniendo como resultado dos indicadores clave: el número de elementos fallados por evento y el total de usuarios afectados. El procedimiento permitió consolidar una base consistente de 95{.}280 registros, alineada con la estructura del conjunto de eventos previamente depurado. La Tabla~\ref{tab:muestra_afectacion_evento} ilustra una muestra de esta agregación. El código SQL correspondiente se presenta en el Listado~\ref{lst:sql_afectacion} (Apéndice~\ref{app:codigo}).
    
    \begin{table}[!htbp]
\centering
\small
\caption{Muestra de afectación por evento (BRAE + QA\_TTC1)}
\label{tab:muestra_afectacion_evento}
\begin{tabularx}{0.9\textwidth}{
    >{\raggedright\arraybackslash}p{2.0cm}
    >{\centering\arraybackslash}p{4.6cm}
    >{\centering\arraybackslash}p{3.0cm}
}
\toprule
    \textbf{Id evento} & \textbf{Cantidad elementos fallados} & \textbf{Cantidad usuarios afectados} \\
\midrule
795044 & 33  & 2692  \\
795513 & 162 & 11796 \\
788221 & 10  & 74    \\
788203 & 240 & 3865  \\
789543 & 8   & 45    \\
789719 & 109 & 503   \\
789929 & 109 & 503   \\
788758 & 1   & 6     \\
798712 & 159 & 1869  \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Extracción del elemento causante histórico}
\label{sec:extraccion_oms_historico}
Con el objetivo de identificar el elemento de la red asociado a la causa de cada evento de maniobra, se realizó la extracción de información histórica desde la base de datos del sistema OMS (Outage Management System), específicamente desde la tabla \texttt{OMS.MANIOBRAS@SPARD}. Esta fuente permite complementar la información de eventos de interrupción mediante la identificación del elemento físico que originó la falla.

El proceso de extracción se centró en dos variables principales: el código de la maniobra (\texttt{CODE}), utilizado como identificador del evento en el sistema OMS, y el elemento fallado (\texttt{BREAKER}), que corresponde al activo de la red asociado a la causa de la interrupción. Estas variables permiten establecer la relación entre el evento registrado y el componente físico responsable dentro de la infraestructura eléctrica.

Adicionalmente, se aplicaron criterios de filtrado temporal y de calidad de datos. En primer lugar, se restringió la información a registros con fecha igual o posterior al año 2021, garantizando la consistencia histórica del análisis. En segundo lugar, se excluyeron aquellos eventos clasificados como pruebas operativas mediante la condición \texttt{CAUSA != 'PRUEBA'}, con el fin de conservar únicamente eventos reales de operación del sistema.

    Como resultado de este proceso de extracción y depuración, se obtuvo una base de datos estructurada a nivel de maniobra, la cual permite complementar la trazabilidad entre eventos de interrupción y los elementos físicos de la red asociados a su ocurrencia. La Tabla~\ref{tab:muestra_elemento_causante_historico} muestra un extracto de esta relación maniobra--elemento. El código SQL correspondiente se presenta en el Listado~\ref{lst:sql_oms_historico} (Apéndice~\ref{app:codigo}).
    
    \begin{table}[!htpb]
\centering
\small
\caption{Muestra de elemento causante histórico (OMS SPARD)}
\label{tab:muestra_elemento_causante_historico}
\begin{tabularx}{0.72\textwidth}{>{\centering\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{4.0cm}}
\toprule
    \textbf{Código maniobra} & \textbf{Elemento fallado} \\
\midrule
640011 & AMVE271 \\
634924 & LMVEL79620 \\
634978 & LMVEL67393 \\
631108 & RMVEL79067-2 \\
631464 & CMVE2454 \\
631761 & SW6844 \\
637873 & BMVE749 \\
640442 & LMVEL99259 \\
631793 & SANC52 \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Extracción del elemento causante actual}
\label{sec:extraccion_oms_actual}
Con el objetivo de actualizar la información histórica del sistema de maniobras, se realizó la extracción de los datos correspondientes a la versión más reciente del sistema OMS en formato plano (CSV). Este proceso permite complementar la información histórica con registros operativos actuales, garantizando la continuidad y consistencia de la base de datos utilizada en el análisis.

La extracción se realizó mediante un proceso de lectura y depuración de datos en Python, a partir del archivo fuente correspondiente a la versión actual del sistema. En primer lugar, se cargó el archivo mediante la función \texttt{read\_csv}, asegurando la correcta lectura de la estructura tabular. Posteriormente, se realizó una limpieza de los nombres de las columnas mediante la eliminación de espacios en blanco al inicio y al final, con el fin de evitar inconsistencias en la identificación de variables.

A continuación, se seleccionaron únicamente las variables relevantes para el análisis: \texttt{Maniobra Apertura}, \texttt{Tipo Elemento} y \texttt{Elemento\_Falla}. Esta selección permitió homogenizar la estructura de la base con la información histórica previamente construida. Finalmente, se eliminaron los registros duplicados mediante la función \texttt{drop\_duplicates}, garantizando la unicidad de las observaciones a nivel de combinación de variables relevantes.

    Como resultado, se obtuvo una base depurada y estandarizada del sistema OMS actual, la cual es consistente con la estructura de la base histórica y permite su integración en el proceso de consolidación del dataset final. La Tabla~\ref{tab:muestra_elemento_causante_actual} presenta una muestra de los registros resultantes. El fragmento Python correspondiente se presenta en el Listado~\ref{lst:py_oms_sp7} (Apéndice~\ref{app:codigo}).

    \begin{table}[!htbp]
\centering
\small
\caption{Muestra de elemento causante actual (OMS SP7)}
\label{tab:muestra_elemento_causante_actual}
\begin{tabularx}{0.95\textwidth}{
    >{\centering\arraybackslash}p{3.8cm} 
    >{\raggedright\arraybackslash}p{4.8cm} 
    >{\centering\arraybackslash}p{2.7cm}}
\toprule
    \textbf{Maniobra apertura} & \textbf{Tipo elemento} & \textbf{Elemento falla} \\
\midrule
424000000000114424 & Transformador de distribucion & 1T07942 \\
124000000000269842 & Fusible & F1979 \\
124000000000269842 & Transformador de distribucion & F1979 \\
124000000000273378 & Fusible & F2004 \\
124000000000269851 & Fusible & F2000 \\
124000000000273383 & Fusible & F2001 \\
124000000000273374 & Fusible & F2006 \\
124000000000273377 & Fusible & F2004 \\
124000000000273372 & Fusible & F2006 \\
124000000000273356 & Fusible & F2005 \\
124000000000273381 & Fusible & F1993 \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Extracción de inventario GIS}
\label{sec:extraccion_gis}
Con el propósito de incorporar la dimensión espacial de la red eléctrica, se realizó la extracción del inventario georreferenciado a partir del sistema GIS corporativo. Esta información permite asociar cada activo de la red con sus coordenadas geográficas y características físicas, facilitando posteriormente el análisis espacial de los eventos de interrupción.

La extracción se efectuó desde la base de datos \texttt{ccomun@gtech}, la cual contiene el inventario técnico de los activos de la red. A partir de esta fuente, se seleccionaron las variables correspondientes al identificador del elemento (\texttt{nodo\_transform\_v}), la latitud (\texttt{coor\_gps\_lat}), la longitud (\texttt{coor\_gps\_lon}) y la altitud (\texttt{coor\_z}). Adicionalmente, se incluyó la variable \texttt{ubicacion}, que permite complementar la caracterización espacial del activo.

Asimismo, se definió explícitamente el tipo de activo como \texttt{Transformador}, con el fin de estandarizar la clasificación dentro del conjunto de datos integrado. Esta homogenización resulta clave para la posterior vinculación con los eventos de interrupción y la construcción del panel activo–día.

    Como resultado, se obtuvo una base georreferenciada de los activos de la red, la cual constituye un insumo fundamental para el enriquecimiento espacial del dataset consolidado. La Tabla~\ref{tab:muestra_inventario_gis} ilustra la estructura de este inventario. El código SQL correspondiente se presenta en el Listado~\ref{lst:sql_gis} (Apéndice~\ref{app:codigo}); la consulta se replica de forma análoga para Interruptor, Cuchilla, Aisladero, Reconectador y Transformador.
    
    \begin{table}[!htbp]
    \centering
    \footnotesize
    \setlength{\tabcolsep}{3pt}
    \renewcommand{\arraystretch}{1.05}
    \caption{Muestra de inventario georreferenciado de activos (GIS GTECH)}
    \label{tab:muestra_inventario_gis}
    \begin{tabularx}{\textwidth}{
    >{\raggedright\arraybackslash}p{1.5cm}
    >{\centering\arraybackslash}p{2.2cm}
    >{\centering\arraybackslash}p{2.2cm}
    >{\centering\arraybackslash}p{1.2cm}
    >{\centering\arraybackslash}X
    }
    \toprule
    \textbf{Elemento} & \textbf{Latitud} & \textbf{Longitud} & \textbf{Altitud} & \textbf{Tipo} \\
    \midrule
    2T01558 & 6.960719612 & -72.18029066 & 636 & Transformador \\
    2T01557 & 6.972762106 & -72.18726993 & 743 & Transformador \\
    2T01467 & 6.973200989 & -72.15472768 & 815 & Transformador \\
    2T01466 & 6.978510343 & -72.14903608 & 726 & Transformador \\
    F3199   & 6.978980206 & -72.19049387 & 832 & Aisladero \\
    PSW8910 & 6.978980206 & -72.19049387 & 832 & Aisladero \\
    2T01555 & 6.982222682 & -72.19495813 & 825 & Transformador \\
    2T01554 & 6.982742594 & -72.20226264 & 867 & Transformador \\
    2T01556 & 6.984663810 & -72.18619273 & 727 & Transformador \\
    2T01465 & 6.985402944 & -72.13537905 & 530 & Transformador \\
    F3192   & 6.987527091 & -72.20044897 & 767 & Aisladero \\
    PSW8909 & 6.987527091 & -72.20044897 & 767 & Aisladero \\
    F5217   & 6.991040416 & -72.20123180 & 726 & Aisladero \\
    PSW8908 & 6.991040416 & -72.20123180 & 726 & Aisladero \\
    2T01553 & 6.991242376 & -72.19636467 & 679 & Transformador \\
    2T01470 & 6.991396285 & -72.15644527 & 790 & Transformador \\
    ESW6924 & 6.992103744 & -72.13486380 & 453 & Aisladero \\
    F5211   & 6.992103744 & -72.13486380 & 453 & Aisladero \\
    2T01471 & 6.992290614 & -72.13485446 & 453 & Transformador \\
    2T01469 & 6.993224220 & -72.14776723 & 656 & Transformador \\
    F8201   & 6.997255476 & -72.12409341 & 361 & Aisladero \\
    \bottomrule
    \end{tabularx}
    \end{table}

\subsection{Extracción de variables meteorológicas}
\label{sec:extraccion_nasa}
    Con el objetivo de incorporar condiciones climáticas como variables explicativas en el análisis de interrupciones del sistema eléctrico, se realizó la extracción de información meteorológica a partir de la plataforma NASA POWER~\textcite{ref48}. Esta fuente proporciona series temporales diarias de variables climáticas relevantes, las cuales permiten capturar la influencia del entorno atmosférico sobre la ocurrencia y duración de eventos en la red.
    
    El proceso de extracción se centró en un conjunto de variables meteorológicas seleccionadas por su relevancia en el análisis de confiabilidad del sistema eléctrico. En particular, se consideraron las variables de precipitación total (\texttt{PRECTOT}), temperatura media (\texttt{T2M}), temperatura máxima (\texttt{T2M\_MAX}), temperatura mínima (\texttt{T2M\_MIN}), humedad relativa (\texttt{RH2M}) y velocidad del viento (\texttt{WS2M}). La Tabla~\ref{tab:muestra_nasa_power_1} presenta una muestra de estas series. La definición de estos parámetros en el código de extracción se presenta en el Listado~\ref{lst:py_nasa} (Apéndice~\ref{app:codigo}).

    \begin{table}[!htbp]
\centering
\small
\caption{Muestra de variables meteorológicas (NASA POWER)}
\label{tab:muestra_nasa_power_1}
\begin{tabularx}{\textwidth}{
>{\centering\arraybackslash}p{2.0cm}
>{\centering\arraybackslash}p{2.0cm}
>{\centering\arraybackslash}p{2.0cm}
>{\centering\arraybackslash}p{2.0cm}
>{\centering\arraybackslash}p{2.0cm}
>{\centering\arraybackslash}p{2.0cm}
}
\toprule
    \textbf{Vel. viento} & \textbf{Temp. media} & \textbf{Humedad} & \textbf{Temp. min} & \textbf{Temp. max} & \textbf{Precip.} \\
\midrule
2.06 & 13.27 & 87.24 & 9.12 & 18.63 & 2.6  \\
2.66 & 13.89 & 88.27 & 10.71 & 18.92 & 1.41 \\
1.93 & 13.66 & 88.59 & 10.29 & 18.96 & 3.04 \\
1.73 & 13.6  & 85.07 & 9.13 & 20.17 & 9.07 \\
1.45 & 11.52 & 69.12 & 5.95 & 18.7  & 0.79 \\
1.1  & 12.29 & 80.8  & 7.55 & 17.76 & 0.77 \\
1.76 & 13.23 & 88.9  & 10.65 & 17.93 & 3.03 \\
2.34 & 12.95 & 88.44 & 10.14 & 17.84 & 2.51 \\
2.41 & 12.69 & 87.38 & 9.48 & 16.55 & 1.84 \\
2.98 & 12.99 & 90.13 & 9.65 & 17.44 & 3.35 \\
3.18 & 13.12 & 87.67 & 10.58 & 17.69 & 0.9  \\
\bottomrule
\end{tabularx}
\end{table}

Estas variables fueron extraídas a nivel diario para el periodo comprendido entre el 1 de enero de 2021 y el 31 de diciembre de 2025, lo que dio lugar a una base de aproximadamente 83.5 millones de registros. Este volumen de información refleja la estructura panelizada de la base climática, construida mediante la combinación de múltiples variables atmosféricas observadas diariamente a lo largo del horizonte temporal de análisis.

La incorporación de estas variables permite enriquecer el dataset consolidado con condiciones exógenas del entorno, facilitando el análisis de su relación con la frecuencia y severidad de las interrupciones en el sistema eléctrico.

\subsection{Resumen metodológico de extracción e integración de bases de datos}
    \begin{table}[!htbp]
\centering
\small
\caption{Resumen metodológico de extracción e integración de bases de datos}
\label{tab:data_lineage}
    \begin{tblr}{
      width=\textwidth,
      colspec={X[l,h] X[l,h] X[l,h] X[l,h]},
      row{1}={font=\bfseries},
      hline{1,2,Z}={solid,0.4pt},
      hline{3-Y}={dotted,0.4pt},
      rowsep=4pt
    }
    Fuente & Variables principales & Proceso aplicado & Resultado \\
    BRAE (TFDDREGISTRO)
      & Id evento, fecha desconexión, fecha conexión
& Filtrado temporal (2021–2025), exclusión de registros no válidos, eliminación de duplicados 
& 95.280 eventos de interrupción \\
BRAE + QA\_TTC1 
      & Cantidad usuarios, elementos de conexión
& JOIN por elemento y periodo, agregación por evento 
      & Base de severidad por evento (usuarios) \\
OMS SPARD (histórico) 
      & Código maniobra, elemento fallado, elemento causante
      & Filtrado de registros desde 2021, exclusión de pruebas
& Base histórica de causa de falla \\
OMS SP7 (actual) 
      & Maniobra apertura, tipo elemento, cantidad elementos fallados, elemento falla
& Lectura CSV, limpieza de columnas, eliminación de duplicados 
& Base actualizada de elementos causantes \\
GIS (GTECH) 
      & Elemento, latitud, longitud, altitud, tipo
& Extracción desde sistema georreferenciado corporativo 
& Inventario espacial de la red \\
NASA POWER 
& PRECTOT, T2M, T2M\_MAX, T2M\_MIN, RH2M, WS2M 
& Extracción diaria (2021–2025) por variables climáticas 
& 83.5 millones de registros meteorológicos \\
    \end{tblr}
\end{table}

    La Tabla~\ref{tab:data_lineage} sintetiza el proceso de extracción e integración desarrollado en la Etapa~1. En conjunto, se consolidaron seis fuentes complementarias: BRAE como base transaccional de eventos de interrupción; BRAE + QA\_TTC1 para cuantificar la severidad operativa (usuarios afectados); OMS SPARD y OMS SP7 para identificar el elemento causante y fallado en los periodos pre y post migración del sistema; GIS (GTECH) para incorporar la georreferenciación de activos de red; y NASA POWER para enriquecer el análisis con variables climáticas diarias. Cada fuente fue sometida a reglas explícitas de filtrado, homologación y depuración, lo que permitió obtener bases consistentes y trazables que constituyen el insumo inicial para la integración del panel descrita en la Etapa~2.



\section{Etapa 2 - Integración y depuración de datos}
\label{sec:metodología-etapa2}

\subsection{Lógica de integración entre sistemas}
\label{sec:logica_integracion}
    Una vez extraídas y depuradas las fuentes descritas en la Etapa~\ref{sec:metodología-etapa1}, la integración se ejecuta de forma secuencial sobre tablas intermedias, preservando trazabilidad de llaves y evitando pérdida de emparejamiento entre sistemas con esquemas distintos (BRAE, OMS SPARD, OMS SP7, GIS y NASA POWER). La secuencia aplicada fue la siguiente.

    {\sloppy
    \renewcommand{\theenumi}{\thesubsection.\arabic{enumi}}
    \renewcommand{\labelenumi}{\theenumi}
\begin{enumerate}
        \item\label{paso:int-1} \textit{Construcción de la tabla base de eventos (BRAE).}\mbox{}\par\vspace{0.2em}\noindent
        A partir de \texttt{BRAE.QA\_TFDDREGISTRO} se genera la tabla \texttt{tabla\_eventos} con \texttt{ID\_EVENTO}, \texttt{FECHA\_DESCONEXION} y \texttt{FECHA\_CONEXION}, aplicando filtros regulatorios del periodo 2021--2025, exclusión de eventos no válidos y eliminación de duplicados. Esta tabla define el universo analítico de interrupciones (95{.}280 eventos).
    
        \item\label{paso:int-2} \textit{Construcción de la tabla de afectación por evento (Tabla~\ref{tab:muestra_afectacion_evento}).}\mbox{}\par\vspace{0.2em}\noindent
        En la Sección~\ref{sec:extraccion_afectacion} se construye la Tabla~\ref{tab:muestra_afectacion_evento} de afectación por evento, la cual consolida las columnas \texttt{CANTIDAD\_ELEMENTOS} y \texttt{CANTIDAD\_USUARIOS} a nivel de \texttt{ID\_EVENTO}. Su integración a la tabla base consolidada se realiza en el paso~\ref{paso:int-5}.
    
        \item\label{paso:int-3} \textit{Completar \texttt{ELEMENTO\_FALLADO} (OMS SPARD + OMS SP7).}\mbox{}\par\vspace{0.2em}\noindent
        La tabla base de eventos obtenida en la Sección~\ref{sec:extraccion_brae} se construyó con el campo \texttt{ELEMENTO\_FALLADO} vacío, reservado para su completamiento en la integración. Dado el cambio de sistema OMS en el año 2023, la información del elemento causante se extrajo de forma separada en dos fuentes: la Sección~\ref{sec:extraccion_oms_historico} (\emph{Extracción del elemento causante histórico}, OMS SPARD) y la Sección~\ref{sec:extraccion_oms_actual} (\emph{Extracción del elemento causante actual}, OMS SP7). En este paso, la tabla de la Sección~\ref{sec:extraccion_brae} se cruza con ambas tablas OMS, aplicando en cada caso la lógica de emparejamiento definida en su extracción:
        
        \begin{itemize}
            \item \textit{Histórico (Sección~\ref{sec:extraccion_oms_historico} -- OMS SPARD):} cruce por \texttt{CODIGO\_MANIOBRA} $\leftrightarrow$ \texttt{ID\_EVENTO} para completar \texttt{ELEMENTO\_FALLADO} con el valor de \texttt{BREAKER} en eventos del periodo pre-migración.
            
            \item \textit{Actual (Sección~\ref{sec:extraccion_oms_actual} -- OMS SP7):} cruce por \texttt{MANIOBRA\_APERTURA} $\leftrightarrow$ \texttt{ID\_EVENTO} para completar \texttt{ELEMENTO\_FALLADO} con el valor de \texttt{ELEMENTO\_FALLA}, junto con \texttt{TIPO\_ELEMENTO}, en eventos del periodo post-migración.
        \end{itemize}
    
        \item\label{paso:int-4} \textit{Construcción de la tabla base consolidada (activo--día).}\mbox{}\par\vspace{0.2em}\noindent
        A partir de este paso se inicia la construcción de la tabla base consolidada del proyecto. Como insumo inicial se toma la Tabla~\ref{tab:muestra_inventario_gis} (inventario georreferenciado de activos obtenido en la Sección~\ref{sec:extraccion_gis}), del cual se conserva un registro por \texttt{ELEMENTO} ($47{.}649$ activos únicos), y se cruza con un calendario diario comprendido entre el 2021-01-01 y el 2025-12-31 ($1{.}826$ días), empleando el campo \texttt{FECHA\_DIA}. Este cruce genera un producto cartesiano entre el universo de activos eléctricos y el horizonte temporal de análisis, con $87{.}007{.}074$ observaciones ($47{.}649 \times 1{.}826$), definiendo la estructura panel \textit{activo $\times$ día} sobre la cual, en los pasos~\ref{paso:int-5} a~\ref{paso:int-8}, se anexan los valores restantes de la secuencia de integración. El detalle del procedimiento para los pasos~\ref{paso:int-4} a~\ref{paso:int-8} se documenta en el Apéndice~\ref{app:html-consolidada}.
    
        \item\label{paso:int-5} \textit{Integración de \texttt{CANTIDAD\_ELEMENTOS} y \texttt{CANTIDAD\_USUARIOS} (afectación por evento).}\mbox{}\par\vspace{0.2em}\noindent
        Sobre la tabla base consolidada del paso~\ref{paso:int-4}, se anexan las métricas de \textit{severidad del evento} (\texttt{ELEMENTOS\_AFECTADOS} y \texttt{USUARIOS\_AFECTADOS} de \texttt{tabla\_eventos\_ajustado.csv}, alineadas con la Tabla~\ref{tab:muestra_afectacion_evento}). Estas cantidades se miden a nivel de \texttt{ID\_EVENTO}, tamaño del impacto de la interrupción, y se atribuyen al activo fallado mediante las llaves:
        \begin{itemize}
            \item \texttt{COMPONENTE\_FALLADO} $=$ \texttt{ELEMENTO} (activo de red, no el identificador del evento);
            \item \texttt{as.Date(FECHA\_DESCONEXION)} $=$ \texttt{FECHA\_DIA}.
        \end{itemize}
        El cruce es izquierdo por (\texttt{ELEMENTO}, \texttt{FECHA\_DIA}) y conserva $N = 87{.}007{.}074$ filas. Si un activo registra varios eventos el mismo día, se conserva un único valor de afectación (el primer \texttt{ID\_EVENTO} ordenado); \textit{no} se suman las severidades de esos eventos en el panel diario. La frecuencia de todas las desconexiones del día se construye por separado en el paso~\ref{paso:int-7}. En días sin falla, \texttt{CANTIDAD\_ELEMENTOS = 0} y \texttt{CANTIDAD\_USUARIOS = 0}.
        
        \noindent\textit{Nota metodológica:} bajo esta regla, \texttt{CANTIDAD\_*} no representa ``usuarios del activo'', sino la severidad del evento anclada al breaker en el día de inicio. En la taxonomía de modelado (Sección~\ref{sec:metodos-modelos}), estas cantidades pertenecen al \textit{impacto posterior}: describen severidad una vez ocurrido el evento y \textit{no} son predictoras contemporáneas de frecuencia o duración. Definiciones alternativas de agregación, suma o máximo de todos los eventos del mismo activo--día, o conteo de usuarios únicos por cuadrante--semana, pueden reelaborarse en la Etapa~4 sin alterar ese rol.
    
        \item\label{paso:int-6} \textit{Integración de variables climáticas (NASA POWER).}\mbox{}\par\vspace{0.2em}\noindent
        Sobre la tabla resultante del paso~\ref{paso:int-5} ($N = 87{.}007{.}074$ filas), se anexan las variables de la serie diaria \texttt{nasa\_power\_consolidado} mediante cruce izquierdo por (\texttt{ELEMENTO}, \texttt{FECHA\_DIA}), incorporando precipitación, temperatura (media, máxima, mínima), humedad relativa y velocidad del viento, sin modificar el cardinal del panel.
    
        \item\label{paso:int-7} \textit{Agregación de variables objetivo (\texttt{DURACION\_min} y \texttt{FRECUENCIA}).}\mbox{}\par\vspace{0.2em}\noindent
        En este paso se construyen las columnas \texttt{DURACION\_min} y \texttt{FRECUENCIA} sobre la tabla resultante del paso~\ref{paso:int-6} ($N = 87{.}007{.}074$ filas). Para ello, se expanden previamente los eventos de la Tabla~\ref{tab:muestra_eventos_brae} (Sección~\ref{sec:extraccion_brae}) a nivel (\texttt{COMPONENTE\_FALLADO}, \texttt{FECHA\_DIA}) y el resultado se integra al panel mediante cruce izquierdo por (\texttt{ELEMENTO}, \texttt{FECHA\_DIA}). La Tabla~\ref{tab:muestra_eventos_brae} contiene los campos \texttt{FECHA\_DESCONEXION} y \texttt{FECHA\_CONEXION}. Con base en las fechas de desconexión y reconexión, se asignan los valores diarios de la siguiente forma:
        \begin{itemize}
            \item \textit{Duración} (\texttt{DURACION\_min}, en minutos): minutos de interrupción del activo \textit{en} \texttt{FECHA\_DIA}, prorrateados por día. Cuando un evento abarca varios días, su duración se reparte entre cada \texttt{FECHA\_DIA} según los minutos que el evento ocupa dentro de ese día (intersección del intervalo \texttt{[FECHA\_DESCONEXION, FECHA\_CONEXION]} con cada día calendario). El día de inicio recibe los minutos desde \texttt{FECHA\_DESCONEXION} hasta la medianoche; los días intermedios, $1{.}440$~min cada uno; y el día de cierre, los minutos desde la medianoche hasta \texttt{FECHA\_CONEXION}. Si varios eventos ocupan el mismo día, se suman los minutos prorrateados de cada uno. \textit{No} se carga la duración total del evento únicamente en el día de desconexión.
            \item \textit{Frecuencia} (\texttt{FRECUENCIA}): número de desconexiones cuyo inicio (\texttt{FECHA\_DESCONEXION}) ocurre en \texttt{FECHA\_DIA} (no se prorratea; se cuenta solo en el día de inicio).
        \end{itemize}
        \noindent Cuando el activo no presenta interrupciones en un día, se asigna \texttt{DURACION\_min = 0} y \texttt{FRECUENCIA = 0}. El cruce exige que \texttt{COMPONENTE\_FALLADO} coincida con \texttt{ELEMENTO} del inventario \texttt{tipo\_elemento.csv} (Tabla~\ref{tab:muestra_inventario_gis}), el cual emplea el \textit{código de sitio} georreferenciado. Cuando OMS registra el \textit{código del activo} reemplazado en lugar del código de sitio, el emparejamiento falla y el evento no se propaga al panel espacial (Sección~\ref{sec:delimitacion_temporal_modelado}).
    
        \item\label{paso:int-8} \textit{Consolidación del panel final (activo--día).}\mbox{}\par\vspace{0.2em}\noindent
        Sobre la tabla base consolidada definida en el paso~\ref{paso:int-4}, y una vez anexados los valores de los pasos~\ref{paso:int-5} a~\ref{paso:int-7}, se obtiene el dataset final con grano \textit{activo eléctrico $\times$ día} y $87{.}007{.}074$ observaciones ($47{.}649$ activos $\times$ $1{.}826$ días). En días sin falla se registra \texttt{DURACION\_min = 0}, \texttt{FRECUENCIA = 0}, \texttt{CANTIDAD\_ELEMENTOS = 0} y \texttt{CANTIDAD\_USUARIOS = 0}.
    
        La base de datos consolidada se estructura como un panel de datos de tipo activo–tiempo, donde la unidad muestral corresponde a un \textit{activo eléctrico observado diariamente}. Cada observación se identifica mediante el par activo--día dentro del periodo de análisis, lo que permite capturar la evolución temporal de cada activo a lo largo del horizonte de estudio. Con $47{.}649$ activos eléctricos únicos en la Tabla~\ref{tab:muestra_inventario_gis} (inventario GIS, un registro por \texttt{ELEMENTO}) y $1{.}826$ días observados (cinco años de información diaria), el panel balanceado comprende $87{.}007{.}074$ observaciones.
    
    
\end{enumerate}
    }
    
    
\subsection{Diagrama de flujo de la secuencia de integración}
    La secuencia de integración y la estructura del panel final se resumen visualmente en la Figura~\ref{fig:panel_activo_dia} (Apéndice~\ref{app:diagrama_panel}), que especifica las relaciones entre las tablas y las operaciones aplicadas en cada paso.


\subsection{Descripción de la tabla base consolidada}
    La Tabla~\ref{tab:muestra_consolidado_preview} presenta una vista previa de la tabla base consolidada resultante del paso~\ref{paso:int-8}, con grano \textit{activo eléctrico $\times$ día}. La muestra completa, con todas las columnas del panel divididas en bloques temáticos (identificación del activo, severidad, clima y continuidad), se documenta en el Apéndice~\ref{app:muestra-consolidado-completa}.

\begin{table}[!htbp]
\centering
    \small
    \caption{Vista previa de la tabla base consolidada (grano activo--día)}
    \label{tab:muestra_consolidado_preview}
    \begin{tabular}{l l r r r r r}
    \toprule
    \textbf{Id activo} & \textbf{Fecha día} & \textbf{C. elem.} & \textbf{C. usu.} & \textbf{Temp. med.} & \textbf{Precip.} & \textbf{Duración (min)} \\
    \midrule
    AASW4819 & 01/01/2021 & 0   & 0    & 23.91 & 3.19 & 0     \\
    1T12452  & 01/01/2021 & 0   & 0    & 24.58 & 1.31 & 0     \\
    REC21365 & 01/01/2021 & 125 & 2015 & 23.04 & 3.19 & 421.5 \\
    CUC12345 & 01/01/2021 & 0   & 0    & 23.02 & 7.28 & 0     \\
    INT39845 & 02/01/2021 & 0   & 0    & 23.83 & 1.99 & 0     \\
    \bottomrule
    \end{tabular}
    \end{table}

    \noindent\textit{Nota:} los registros sin interrupción en el día presentan \texttt{DURACION\_min = 0} y \texttt{FRECUENCIA = 0}. La duración del panel diario se expresa en \textit{minutos} (columna \texttt{DURACION\_min} en \texttt{data\_consolidada.csv}).
    
\subsection{Descripción de las variables}
    \paragraph{Severidad y clima inicial (Tabla~\ref{tab:muestra_consolidado_preview}; detalle completo en el Apéndice~\ref{app:muestra-consolidado-completa}).}
    Las columnas \texttt{ID ACTIVO} y \texttt{FECHA DIA} se repiten en esta tabla para facilitar la trazabilidad de filas entre bloques consecutivos. Las variables propias de este bloque son:
    
    \begin{itemize}
        \item \textit{C. elem.} (\texttt{CANTIDAD\_ELEMENTOS}): número de elementos de red afectados por el \textit{evento} de interrupción atribuido al activo fallado (\texttt{COMPONENTE\_FALLADO}) en el día de desconexión (paso~\ref{paso:int-5}). No es el inventario del activo ni un conteo de usuarios únicos del cuadrante. Si hay varios eventos el mismo activo--día, el panel conserva la severidad de un solo evento. Cuando el activo no registra falla en el día, el valor es 0. En el modelado es \textit{impacto posterior} (Sección~\ref{sec:metodos-modelos}), no predictora contemporánea. \textit{Clasificación:} cuantitativa discreta.
    
        \item \textit{C. usu.} (\texttt{CANTIDAD\_USUARIOS}): número de usuarios afectados por el mismo evento, con la misma atribución espacial y temporal que \texttt{CANTIDAD\_ELEMENTOS}. Es una medida de severidad operativa del impacto sobre la demanda (impacto posterior; Sección~\ref{sec:metodos-modelos}), no una covariable disponible antes del evento ni una predictora válida de su ocurrencia o duración. En días sin interrupción, el valor es 0. \textit{Clasificación:} cuantitativa discreta.
    
        \item \textit{Vel. viento} (\texttt{WS2M}): velocidad del viento a 2\,m de altura, expresada en m/s. Corresponde a la variable \texttt{WS2M} de NASA POWER (Sección~\ref{sec:extraccion_nasa}) e integra al panel por \texttt{FECHA\_DIA} en el paso~\ref{paso:int-6}. \textit{Clasificación:} cuantitativa continua.
    
        \item \textit{Temp. med.} (\texttt{T2M}): temperatura media del aire en el día, expresada en \textdegree C. Proviene del parámetro \texttt{T2M} de NASA POWER y captura las condiciones térmicas promedio del entorno asociadas a cada observación activo--día. \textit{Clasificación:} cuantitativa continua.
    \end{itemize}
    
        \paragraph{Clima complementario (detalle completo en el Apéndice~\ref{app:muestra-consolidado-completa}).}
        Al igual que en la Tabla~\ref{tab:muestra_consolidado_preview}, \texttt{ID ACTIVO} y \texttt{FECHA DIA} permiten alinear filas entre tablas. Las variables climáticas restantes del bloque son:
        
    \begin{itemize}
        \item \textit{Hum. rel.} (\texttt{RH2M}): humedad relativa del aire a 2\,m, expresada en porcentaje (\%). Proviene del parámetro \texttt{RH2M} de NASA POWER e integra al panel por \texttt{FECHA\_DIA}. \textit{Clasificación:} cuantitativa continua.
    
        \item \textit{Temp. min.} (\texttt{T2M\_MIN}): temperatura mínima diaria del aire, expresada en \textdegree C. Corresponde al parámetro \texttt{T2M\_MIN} de NASA POWER y complementa la temperatura media con el extremo inferior del rango térmico del día. \textit{Clasificación:} cuantitativa continua.
    
        \item \textit{Temp. max.} (\texttt{T2M\_MAX}): temperatura máxima diaria del aire, expresada en \textdegree C. Proviene del parámetro \texttt{T2M\_MAX} de NASA POWER y representa el extremo superior del rango térmico observado en el día. \textit{Clasificación:} cuantitativa continua.
    
        \item \textit{Precip.} (\texttt{PRECTOT}): precipitación total acumulada en el día, expresada en mm. Corresponde al parámetro \texttt{PRECTOT} de NASA POWER y permite incorporar la exposición del activo a condiciones de lluvia en la fecha de observación. \textit{Clasificación:} cuantitativa continua.
    \end{itemize}
    
    \paragraph{Variables de continuidad (Tabla~\ref{tab:muestra_consolidado_preview}).}
    Este bloque concentra las variables objetivo del análisis, construidas en el paso~\ref{paso:int-7} a partir de la Tabla~\ref{tab:muestra_eventos_brae} (eventos depurados, Sección~\ref{sec:extraccion_brae}) y de los campos \texttt{FECHA\_DESCONEXION} y \texttt{FECHA\_CONEXION}:
    
    \begin{itemize}
        \item \textit{Duración} (\texttt{DURACION\_min}, en minutos): minutos de interrupción del activo \textit{en} \texttt{FECHA\_DIA}, obtenidos prorrateando cada evento según la intersección de \texttt{[FECHA\_DESCONEXION, FECHA\_CONEXION]} con ese día (paso~\ref{paso:int-7}). Si el evento cruza varios días, su duración se reparte entre ellos; si hay varios eventos el mismo día, se suman los minutos prorrateados. No equivale a colocar la duración total del evento solo en el día de desconexión. En días sin falla, el valor es $0$. \textit{Clasificación:} cuantitativa continua.
    
        \item \textit{Frecuencia} (\texttt{FRECUENCIA}): número de interrupciones del activo cuyo inicio (\texttt{FECHA\_DESCONEXION}) ocurre en \texttt{FECHA\_DIA}. La frecuencia se contabiliza únicamente en el día de inicio y \textit{no} se prorratea entre días. En días sin falla, el valor es $0$. \textit{Clasificación:} cuantitativa discreta.
    \end{itemize}
    
    
    
    Una vez construido el panel consolidado (pasos de integración~\ref{paso:int-4}--\ref{paso:int-8}), se deriva la tabla agregada por cuadrante y semana (Sección~\ref{sec:agrupacion_cuadrante_semana}). La Etapa~2 cierra con la evidencia institucional de calidad de las fuentes (Sección~\ref{sec:evidencia_calidad_fuentes}). El diagnóstico de completitud del panel, la identificación de valores extremos y el resto del análisis exploratorio se desarrollan en la Etapa~3; la imputación, el tratamiento definitivo de atípicos y la preparación del dataset para modelado, en la Etapa~4.
    
    \subsection{Agrupación de data consolidada por semana y cuadrante}
    \label{sec:agrupacion_cuadrante_semana}
    
    \subsubsection{Definición del grano analítico (cuadrante $\times$ semana)}
    
    El panel consolidado con grano \textit{activo eléctrico $\times$ día} ($N = 87{.}007{.}074$ observaciones) constituye la base de integración y trazabilidad del proyecto: preserva la relación uno a uno entre cada activo de red y cada día del horizonte 2021--2025, y permite reconstruir con rigor la secuencia de pasos descrita en la Sección~\ref{sec:logica_integracion}. No obstante, el objetivo del estudio, pronosticar el comportamiento de $SAIDI$ y $SAIFI$ en distintas áreas del territorio de concesión, exige una unidad de análisis de mayor agregación espacial y temporal, coherente con la lectura regulatoria y operativa de los indicadores de continuidad del servicio.
    
    Desde el punto de vista espacial, los indicadores de calidad se interpretan y gestionan a escala territorial (zonas de concentración, circuitos y áreas de servicio), no a nivel de un activo aislado. Para sintetizar el comportamiento de la red en unidades homogéneas del territorio, se adopta la malla institucional de \textit{800 cuadrantes} que cubre el área de concesión de la red eléctrica. En esta malla, el sufijo \texttt{\_800} identifica la versión del esquema con 800 divisiones espaciales; cada celda tiene dimensiones aproximadas de $5 \times 2{.}5$\,km. Cada activo eléctrico se asigna a un \texttt{id\_cuadrante} mediante el catálogo \texttt{ELEMENTO\_FALLA\_CUADRANTE\_800}, de modo que las observaciones diarias de múltiples activos ubicados en la misma celda puedan consolidarse en una métrica zonal. La Figura~\ref{fig:cuadrantes_800} ilustra la estructura de esta partición espacial sobre el área de estudio.
    
    \begin{figure}[!htbp]
\centering
        \includegraphics[width=0.95\textwidth]{img/cuadrantes_800.png}
        \caption{Malla de 800 cuadrantes sobre el territorio de concesión (celdas de aprox.\ $5 \times 2{.}5$\,km).}
        \label{fig:cuadrantes_800}
    \end{figure}
    
    Desde el punto de vista temporal, la granularidad diaria del panel es necesaria para la integración de eventos, clima y severidad, pero resulta excesivamente ruidosa para el análisis exploratorio espacial y para la modelación orientada a dinámicas semanales de continuidad y clima. Por ello, se define la \textit{semana calendario} (con inicio en lunes) como periodo de agregación temporal, lo que permite suavizar fluctuaciones diarias, estabilizar las series de interrupción y alinear las covariables meteorológicas en un horizonte compatible con la gestión operativa y con visualizaciones espacio-temporales (mapas por cuadrante y series semanales).
    
    Bajo esta definición, la unidad muestral del dataset agregado es el par \textit{(cuadrante, semana)}. El panel \texttt{data\_consolidada.csv} se conserva como insumo base y referencia del proyecto; la tabla derivada \texttt{data\_agrupada\_por\_cuadrante\_semana.csv} constituye la representación analítica para los análisis territoriales posteriores, sin sustituir ni alterar el panel diario original.
    
    \subsubsection{Procedimiento de agregación}
    
    Con el panel consolidado \texttt{data\_consolidada.csv}, se construyó la tabla de análisis a grano \textit{cuadrante $\times$ semana}. El procedimiento se documenta en el Apéndice~\ref{app:html-agrupacion}.
    
    El insumo principal es el panel del paso~\ref{paso:int-8}. Para asignar cada activo a su zona de análisis espacial, se utiliza el catálogo \texttt{ELEMENTO\_FALLA\_CUADRANTE\_800}, del cual se extrae la relación \texttt{ELEMENTO} $\rightarrow$ \texttt{id\_cuadrante} (campo \texttt{CUADRANTE\_1}). La asignación se integra al panel mediante cruce izquierdo por \texttt{ELEMENTO}, conservando todas las filas del panel y anexando el identificador de cuadrante cuando existe emparejamiento. Complementariamente, el archivo \texttt{CUADRANTE\_CORD\_800} aporta la referencia de centroides por cuadrante.
    
    Sobre el panel enriquecido, cada observación diaria se asigna a una semana calendario mediante \texttt{floor\_date(FECHA\_DIA, unit = ``week'', week\_start = 1)}, de modo que la semana inicia en lunes. La duración del panel activo~$\times$~día se conserva en \textit{minutos} (\texttt{DURACION\_min}; paso~\ref{paso:int-7}). Para la agregación semanal se convierte a horas como \texttt{duracion\_horas = DURACION\_min / 60}. Esta conversión \textit{no} vuelve a prorratear entre días: el prorrateo temporal ya se resolvió al construir el panel diario; aquí solo se unifican unidades y se suman los minutos diarios (ya prorrateados) de los activos del cuadrante.
    
    La agregación distingue dos naturalezas de variable. Las variables \textit{operativas} (impacto y continuidad) son propias de cada activo: dos activos del mismo cuadrante pueden fallar el mismo día y sus conteos deben sumarse, por lo que se agregan directamente sobre las filas activo~$\times$~día con \texttt{group\_by(id\_cuadrante, semana)}. Las variables \textit{climáticas}, en cambio, describen un campo espacial común: como consecuencia del cruce del paso~\ref{paso:int-6}, todos los activos de un mismo cuadrante comparten el valor meteorológico del día, de modo que la serie diaria aparece replicada tantas veces como activos tenga el cuadrante. Por ello el bloque climático se agrega en \textit{dos etapas}: primero se colapsa el panel a un registro por (\texttt{id\_cuadrante}, \texttt{FECHA\_DIA}), eliminando la replicación por activo, y sobre esa serie diaria del cuadrante se calcula el resumen semanal. Omitir la primera etapa dejaría el acumulado de lluvia multiplicado por el número de activos del cuadrante, un sesgo multiplicativo que además varía entre cuadrantes y contamina cualquier comparación territorial posterior.

    Las reglas de resumen son las siguientes:
    
    \begin{itemize}
        \item \textit{Coordenadas del cuadrante} (\texttt{latitud}, \texttt{longitud}, \texttt{altitud}): primer valor registrado en el grupo (\texttt{first}), tomado del inventario georreferenciado del panel.
        
        \item \textit{Impacto operativo:} \texttt{usuarios\_afectados} $=$ \texttt{sum(CANTIDAD\_USUARIOS)} y \texttt{elementos\_afectados} $=$ \texttt{sum(CANTIDAD\_ELEMENTOS)} en la semana, sumando las severidades de evento ya atribuidas a los activos del cuadrante (paso~\ref{paso:int-5}). Esta suma agrega \textit{impactos de evento}, no un conteo de usuarios o elementos únicos de la semana; tampoco multiplica la severidad por el número de activos del cuadrante (a diferencia del clima, que sí requería colapsar la replicación diario--activo). En el modelado pertenecen al grupo de \textit{impacto posterior} (Sección~\ref{sec:metodos-modelos}): se conservan para el EDA y la descripción de severidad, pero \textit{no} entran como predictoras contemporáneas de \texttt{n\_eventos} ni de \texttt{duracion\_total}. Una reelaboración de la regla de agregación, si procede, se documenta en la Etapa~4 sin cambiar esa exclusión.
        
        \item \textit{Variables climáticas semanales:} sobre la serie diaria por cuadrante de la primera etapa, \texttt{temp\_media\_semana}, \texttt{humedad\_media\_semana} y \texttt{viento\_medio\_semana} como promedio de los días de la semana; \texttt{temp\_max\_semana} y \texttt{viento\_max\_semana} como máximo; \texttt{temp\_min\_semana} como mínimo; y \texttt{precip\_total\_semana} como suma de la precipitación diaria del cuadrante, es decir el acumulado de lluvia de la semana en milímetros.
        
        \item \textit{Continuidad del servicio:} \texttt{n\_eventos} $=$ \texttt{sum(FRECUENCIA)} (total de desconexiones \textit{iniciadas} en la semana, sin prorrateo de la frecuencia) y \texttt{duracion\_total} $=$ \texttt{sum(duracion\_horas)} (suma semanal, en horas, de los minutos diarios de interrupción ya prorrateados en el panel activo~$\times$~día; unidad resultante: horas).
    \end{itemize}
    
    El resultado se exporta como \texttt{data\_agrupada\_por\_cuadrante\_semana.csv} en la ruta definida por la variable de entorno \texttt{UBICACION\_DATA}.
    
\subsection{Descripción de la tabla base semanal cuadrante}
    La Tabla~\ref{tab:base_semanal_cuadrante} resume la estructura del dataset agregado \texttt{data\_agrupada.csv}, con grano \textit{cuadrante $\times$ semana}. Cada fila corresponde a un par (\texttt{id\_cuadrante}, \texttt{semana}) y concentra, para esa celda de la malla y ese periodo semanal, las coordenadas de referencia, el impacto operativo acumulado, las variables climáticas resumidas y las métricas de continuidad del servicio. Se incluyen además los bloques incorporados en la materialización de $D_{\mathrm{mod}}$ (\texttt{data\_agrupada\_mod.csv}): usuarios y denominadores, composición de estrato/sector, fechas y reposiciones de transformadores, y las respuestas SAIDI/SAIFI. La Tabla~\ref{tab:muestra_semanal_preview} presenta una vista previa; la muestra completa por bloques temáticos se documenta en el Apéndice~\ref{app:muestra-semanal-completa}.
    
    \begin{table}[!htbp]
    \centering
    \footnotesize
    \setlength{\tabcolsep}{4pt}
    \caption{Estructura de la tabla base semanal cuadrante (\texttt{data\_agrupada.csv}).}
    \label{tab:base_semanal_cuadrante}
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{2.4cm} >{\raggedright\arraybackslash}p{5.6cm} >{\raggedright\arraybackslash}p{2.6cm}@{}}
\toprule
    \textbf{Bloque} & \textbf{Variables} & \textbf{Regla de agregación} \\
\midrule
    Identificación &
    \begin{minipage}[t]{\linewidth}
    \texttt{id\_cuadrante}\\[0.1em]
    \texttt{semana}
    \end{minipage} &
    Llave del grupo \\
    Georreferencia &
    \begin{minipage}[t]{\linewidth}
    \texttt{latitud}\\[0.1em]
    \texttt{longitud}\\[0.1em]
    \texttt{altitud}
    \end{minipage} &
    \texttt{first} \\
    Impacto &
    \begin{minipage}[t]{\linewidth}
    \texttt{usuarios\_afectados}\\[0.1em]
    \texttt{elementos\_afectados}
    \end{minipage} &
    \texttt{sum} \\
    Clima &
    \begin{minipage}[t]{\linewidth}
    \texttt{temp\_media\_semana}\\[0.1em]
    \texttt{temp\_max\_semana}\\[0.1em]
    \texttt{temp\_min\_semana}\\[0.1em]
    \texttt{humedad\_media\_semana}\\[0.1em]
    \texttt{viento\_medio\_semana}\\[0.1em]
    \texttt{viento\_max\_semana}\\[0.1em]
    \texttt{precip\_total\_semana}
    \end{minipage} &
    \begin{minipage}[t]{\linewidth}
    \texttt{mean}$^{\dagger}$\\[0.1em]
    \texttt{max}$^{\dagger}$\\[0.1em]
    \texttt{min}$^{\dagger}$\\[0.1em]
    \texttt{mean}$^{\dagger}$\\[0.1em]
    \texttt{mean}$^{\dagger}$\\[0.1em]
    \texttt{max}$^{\dagger}$\\[0.1em]
    \texttt{sum}$^{\dagger}$
    \end{minipage} \\
    Continuidad &
    \begin{minipage}[t]{\linewidth}
    \texttt{n\_eventos}\\[0.1em]
    \texttt{duracion\_total}
    \end{minipage} &
    \texttt{sum} \\
    Usuarios$^{\ddagger}$ &
    \begin{minipage}[t]{\linewidth}
    \texttt{usuarios\_totales}\\[0.1em]
    \texttt{cantidad\_usuarios\_cuadrante}
    \end{minipage} &
    Cruce por mes / cuadrante \\
    Estrato y sector$^{\ddagger}$ &
    \begin{minipage}[t]{\linewidth}
    \texttt{ESTRATO\_1} \ldots{} \texttt{ESTRATO\_6}\\[0.1em]
    \texttt{INDUSTRIAL}, \texttt{COMERCIAL}\\[0.1em]
    \texttt{OFICIAL}, \texttt{PROVISIONAL}\\[0.1em]
    \texttt{ALUMBRADO\_PUBLICO}
    \end{minipage} &
    Conteo por cuadrante \\
    Activos$^{\ddagger}$ &
    \begin{minipage}[t]{\linewidth}
    \texttt{FECHA\_ACTIVO\_MAS\_VIEJO}\\[0.1em]
    \texttt{FECHA\_ACTIVO\_MAS\_NUEVO}\\[0.1em]
    \texttt{PROM\_CANT\_MODIFICACIONES}
    \end{minipage} &
    \begin{minipage}[t]{\linewidth}
    \texttt{min}\\[0.1em]
    \texttt{max}\\[0.1em]
    \texttt{mean}
    \end{minipage} \\
    Respuestas$^{\ddagger}$ &
    \begin{minipage}[t]{\linewidth}
    \texttt{SAIDI}\\[0.1em]
    \texttt{SAIFI}
    \end{minipage} &
    Cálculo derivado \\
    \bottomrule
    \end{tabular}
    
    \noindent\textit{Nota:} $^{\dagger}$ el bloque climático se resume en dos etapas; la regla indicada opera sobre la serie diaria por cuadrante, obtenida tras colapsar las filas activo~$\times$~día que replican el mismo valor meteorológico. $^{\ddagger}$ bloques incorporados en la materialización de $D_{\mathrm{mod}}$ (\texttt{data\_agrupada\_mod.csv}): usuarios y composición de estrato/sector provienen de los maestros comerciales cruzados por mes y cuadrante; las fechas y el promedio de reposiciones se derivan de \texttt{TT2\_FMODIFICACION} por cuadrante; SAIDI y SAIFI se calculan a partir de la duración y frecuencia de interrupciones sobre \texttt{usuarios\_totales} y se emplean exclusivamente como variables de respuesta.
    \end{table}
    
    \begin{table}[!htbp]
    \centering
    \small
    \caption{Vista previa de la tabla base semanal cuadrante (grano cuadrante--semana)}
    \label{tab:muestra_semanal_preview}
    \begin{tabular}{r l r r r r r}
    \toprule
    \textbf{Id cuad.} & \textbf{Semana} & \textbf{Usu. afect.} & \textbf{Temp. med.} & \textbf{Precip.} & \textbf{N eventos} & \textbf{Dur. total (h)} \\
    \midrule
      1 & 28/12/2020 &    0 & 13.6 & 18.5 & 0 &  0.0 \\
     12 & 11/01/2021 &  842 & 22.4 &  8.4 & 5 & 28.4 \\
     45 & 18/01/2021 &    0 & 23.8 &  6.2 & 0 &  0.0 \\
    117 & 25/01/2021 & 1254 & 24.1 &  9.8 & 8 & 41.7 \\
    203 & 01/02/2021 &    0 & 24.5 &  4.1 & 0 &  0.0 \\
    \bottomrule
    \end{tabular}
    \end{table}

    \noindent\textit{Nota:} la muestra completa, con coordenadas, temperaturas máxima/mínima, viento y humedad por bloques temáticos, se documenta en el Apéndice~\ref{app:muestra-semanal-completa}. Cadena de unidades de continuidad: panel diario en \textit{minutos} (\texttt{DURACION\_min}) $\rightarrow$ conversión \texttt{/60} $\rightarrow$ \texttt{duracion\_total} semanal en \textit{horas}. El prorrateo entre días se aplica solo al construir \texttt{DURACION\_min} (paso~\ref{paso:int-7}); la agregación semanal suma esos minutos ya prorrateados, convertidos a horas. Las semanas sin interrupciones registran \texttt{n\_eventos = 0} y \texttt{duracion\_total = 0}. Las unidades de temperatura, humedad, viento y precipitación coinciden con las del panel diario (°C, \%, m/s y mm, respectivamente).


\subsection{Justificación y enriquecimiento del panel cuadrante--semana}
\label{sec:enriquecimiento_panel_cuadrante}
Tras materializar la tabla base semanal (Secciones~\ref{sec:agrupacion_cuadrante_semana}--\ref{tab:base_semanal_cuadrante}), se incorporan atributos territoriales, de red y de exposición que no están en el panel operativo original. El procedimiento reproducible corresponde a los informes \texttt{Etapa2\_03} a \texttt{Etapa2\_12} (Apéndices~\ref{app:html-etapa2-03}--\ref{app:html-etapa2-12}). Cada bloque se argumenta en el cuerpo del capítulo con hallazgos, figuras representativas y remisión al HTML para el detalle de código.

\subsubsection{Justificación de la agregación espacial}
\label{sec:justificacion_agregacion_espacial}
La elección del grano cuadrante~$\times$~semana se fundamenta en la evidencia de concentración espacial de fallas y en la escala de dependencia comparable al tamaño de los cuadrantes de la malla operativa (\texttt{Etapa2\_03\_justificacion\_agregacion\_espacial.Rmd}). Operar a nivel activo~$\times$~día preserva trazabilidad, pero genera esparcidad extrema e inviabilidad computacional para vecindades densas; la agregación reduce ceros estructurales y alinea la unidad de análisis con la lectura territorial de $SAIDI$ y $SAIFI$.

\subsubsection{Infraestructura eléctrica por cuadrante}
\label{sec:infraestructura_cuadrante}
A partir del inventario GIS se agregan conteos y tipologías de elementos (transformadores, seccionadores, etc.) al polígono de cada cuadrante (\texttt{Etapa2\_04}). El resultado se exporta en \texttt{infraestructura\_cuadrantes\_reales.gpkg} y alimenta variables de exposición estática (\texttt{n\_elementos}, proporciones por tipo). La Figura~\ref{fig:infra_hist_elementos} resume la heterogeneidad de escala entre cuadrantes; la Figura~\ref{fig:infra_composicion_top30} ilustra la composición de infraestructura en los cuadrantes con mayor densidad de activos.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{img/infra_hist_elementos.png}
    \caption{Distribución del número de elementos de infraestructura por cuadrante real.}
    \label{fig:infra_hist_elementos}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{img/infra_composicion_top30.png}
    \caption{Composición tipológica de infraestructura en los 30 cuadrantes con mayor número de elementos.}
    \label{fig:infra_composicion_top30}
\end{figure}

\noindent Detalle reproducible: Apéndice~\ref{app:html-etapa2-04}.

\subsubsection{Estratificación municipal}
\label{sec:estratificacion_municipal}
Mediante intersección espacial entre polígonos de cuadrante y la capa oficial DANE de estratificación municipal (\texttt{Mpio\_Metodologia\_Estr\_Estr\_Predom}), se asigna a cada cuadrante el municipio y el estrato predominante (\texttt{Etapa2\_05}). En límites administrativos la regla de desempate es el municipio del centroide. La Figura~\ref{fig:estrato_mapa} muestra la asignación territorial; la Figura~\ref{fig:estrato_distribucion} la frecuencia de cuadrantes por estrato predominante.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{img/estrato_mapa_estatico.png}
    \caption{Mapa de cuadrantes según estrato socioeconómico predominante del municipio.}
    \label{fig:estrato_mapa}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{img/estrato_distribucion.png}
    \caption{Distribución de cuadrantes por estrato predominante municipal.}
    \label{fig:estrato_distribucion}
\end{figure}

\noindent Se reconoce que el estrato es una variable \textit{municipal}, no predial dentro del cuadrante. Detalle: Apéndice~\ref{app:html-etapa2-05}.

\subsubsection{Eventos catastróficos (UNGRD)}
\label{sec:desastres_ungrd}
Los registros de emergencias de la Unidad Nacional para la Gestión del Riesgo de Desastres (UNGRD) se agregan a municipio--semana y se trasladan al cuadrante vía la asignación municipal previa (\texttt{Etapa2\_06}). La Figura~\ref{fig:desastres_serie} describe la dinámica temporal de eventos catastróficos; la Figura~\ref{fig:desastres_tipo} la frecuencia por tipo de emergencia.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{img/desastres_serie_temporal.png}
    \caption{Serie temporal de eventos catastróficos UNGRD asociados al área de estudio.}
    \label{fig:desastres_serie}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{img/desastres_frecuencia_tipo.png}
    \caption{Frecuencia de eventos catastróficos por tipo (UNGRD).}
    \label{fig:desastres_tipo}
\end{figure}

\noindent Detalle: Apéndice~\ref{app:html-etapa2-06}.

\subsubsection{Sismicidad (SGC)}
\label{sec:sismicidad_sgc}
La sismicidad del Servicio Geológico Colombiano se integra como exposición territorial por buffers de distancia al centroide del cuadrante y agregación semanal (\texttt{Etapa2\_07}). La Figura~\ref{fig:sismos_serie} muestra la evolución temporal; la Figura~\ref{fig:sismos_magnitud} la distribución de magnitudes.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{img/sismos_serie_temporal.png}
    \caption{Serie temporal de sismos considerados en la exposición territorial del panel.}
    \label{fig:sismos_serie}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{img/sismos_histograma_magnitud.png}
    \caption{Histograma de magnitudes de sismos integrados al panel cuadrante--semana.}
    \label{fig:sismos_magnitud}
\end{figure}

\noindent Detalle: Apéndice~\ref{app:html-etapa2-07}.

\subsubsection{Topología de red eléctrica}
\label{sec:topologia_red}
Sobre tramos y nodos de la red se construyen métricas por cuadrante (longitud de red, densidad de nodos terminales, circuitos) y una matriz de vecindad eléctrica/espacial para rezagos entre vecinos (\texttt{Etapa2\_08}). La Figura~\ref{fig:red_longitud} describe la heterogeneidad de longitud de red; la Figura~\ref{fig:red_saidi} relaciona preliminarmente longitud de red y $SAIDI$.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{img/red_longitud_distribucion.png}
    \caption{Distribución de la longitud de red eléctrica por cuadrante.}
    \label{fig:red_longitud}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{img/red_saidi_vs_longitud.png}
    \caption{Relación exploratoria entre longitud de red y $SAIDI$ por cuadrante.}
    \label{fig:red_saidi}
\end{figure}

\noindent Detalle: Apéndice~\ref{app:html-etapa2-08}.

\subsubsection{Integración del panel enriquecido y series SAIDI/SAIFI}
\label{sec:integracion_panel_enriquecido}
Los bloques anteriores se unen al panel base por \texttt{id\_cuadrante} y \texttt{semana} (\texttt{Etapa2\_09}--\texttt{Etapa2\_12}): clima NASA semanal, eventos de continuidad, NDVI y atributos estáticos. El artefacto resultante \texttt{data\_final\_modelo\_cuadrantes\_reales.rds} es el insumo de las Etapas~4 y~5. La Figura~\ref{fig:panel_series_saidi_saifi} muestra la coevolución temporal de $SAIDI$ y $SAIFI$; la Figura~\ref{fig:panel_ndvi_saidi} ilustra la relación exploratoria NDVI--$SAIDI$.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{img/panel_series_saidi_saifi.png}
    \caption{Series temporales de $SAIDI$ y $SAIFI$ en el panel enriquecido cuadrante--semana.}
    \label{fig:panel_series_saidi_saifi}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{img/panel_ndvi_vs_saidi.png}
    \caption{Relación exploratoria entre NDVI semanal y $SAIDI$ en el panel enriquecido.}
    \label{fig:panel_ndvi_saidi}
\end{figure}

\noindent Detalle reproducible: Apéndices~\ref{app:html-etapa2-09}--\ref{app:html-etapa2-12}.

\subsection{Evidencia de calidad y confiabilidad de las fuentes}
\label{sec:evidencia_calidad_fuentes}
    La validación de los datos recopilados por Centrales Eléctricas de Norte de Santander S.A. E.S.P. (CENS) se fundamenta en su arquitectura institucional de gestión, enmarcada en la Política de Gestión de Tecnología de la Información (Sesión de la Junta Directiva No.~800 del 20 de febrero de 2018)~\textcite{ref38} y en la Política del Sistema de Gestión Integrado (Sesión de la Junta Directiva No.~831 del 22 de abril de 2020)~\textcite{ref39}, orientadas a garantizar una gestión sistemática de calidad continua, sostenibilidad ambiental y cumplimiento legal mediante la evaluación permanente de la calidad de los datos, priorizando consistencia, disponibilidad y accesibilidad de la información.
    
    CENS refuerza su gobernanza de datos mediante Instrumentos de Gestión de Información Pública, entre los cuales se destacan el Registro de Activos de Información (IPR), utilizado para detectar, clasificar y almacenar conjuntos de datos institucionales; los Cuadros de Clasificación Documental; el Esquema de Publicación de Información (IPS); el Índice de Información Clasificada y Reservada; los Costos de Reproducción; y el Programa de Gestión Documental (PGD). Estos instrumentos enmarcan el proceso institucional de almacenamiento, organización, acceso, difusión, protección y preservación de la información~\textcite{ref40}.
    
    En conjunto, estos mecanismos reflejan sistemas de control interno, gobernanza, confianza en la información documentada y transparencia institucional. Asimismo, evidencian el funcionamiento estructurado de los registros y su disponibilidad operativa, garantizando la calidad, veracidad y consistencia de los datos utilizados en este estudio para la toma de decisiones analíticas y gerenciales.
    
    La fiabilidad técnica de las variables climáticas se sustenta en NASA POWER, plataforma del Langley Research Center de la NASA de la cual provienen las series meteorológicas integradas al panel (Sección~\ref{sec:metodología-etapa1}). Esta fuente fue seleccionada por ofrecer cobertura diaria completa y homogénea sobre la totalidad del área de servicio durante el periodo de estudio, sin los vacíos ni las discontinuidades propias de una red de estaciones terrestres.
    
    Los parámetros utilizados se derivan del reanálisis MERRA-2 del programa Earth Science de la NASA y se distribuyen mediante una API pública, documentada y versionada, con definiciones y unidades explícitas para cada variable. Estas características garantizan trazabilidad, reproducibilidad y consistencia metodológica en la captura de las series climáticas, de modo que la extracción documentada en la Etapa~1 puede replicarse de forma íntegra~\textcite{ref48}.
    




\section{Etapa 3 – Análisis descriptivo y exploratorio de datos (EDA)}
\label{sec:metodología-etapa3}

Una vez completadas las etapas de extracción e integración (Etapas~1 y~2), se desarrolla un análisis exploratorio orientado a caracterizar las distribuciones de las variables, diagnosticar la calidad del panel (valores faltantes y extremos), identificar patrones temporales y espaciales, y evaluar las relaciones entre covariables que podrían explicar la variación de $SAIDI$ y $SAIFI$. Esta etapa se concibe como un análisis diagnóstico previo al modelamiento, orientado a garantizar la estabilidad e interpretabilidad de los resultados y a fundamentar las decisiones de imputación y transformación de la Etapa~4.

Para las variables cuantitativas se calculan medidas de tendencia central y de dispersión, incluida la desviación estándar y el coeficiente de variación (CV; Sección~\ref{sec:metodos-modelos}), así como la proporción de valores atípicos; adicionalmente se construyen histogramas y diagramas de caja para examinar la forma de las distribuciones. En el caso de las variables categóricas, se analizan las frecuencias absolutas y relativas de cada categoría. El grado de asociación se evalúa con el coeficiente de Spearman~\textcite{ref53} (cuantitativas) y el estadístico $V$ de Cramér~\textcite{ref54} (cualitativas), según se formalizan en la Sección~\ref{sec:metodos-modelos}, con el fin de identificar redundancias y relaciones preliminares que orienten la depuración de variables en la Etapa~4. Las visualizaciones incluyen series de tiempo, gráficos de dispersión, mapas temáticos y mapas de calor de correlaciones.

Las actividades desarrolladas en esta etapa se describen en las subsecciones siguientes. Tras la caracterización del panel, se documentan el diagnóstico de datos faltantes (Sección~\ref{sec:identificacion_datos_faltantes}) y la identificación de valores extremos (Sección~\ref{sec:identificacion_valores_extremos}); a continuación se profundiza en estadísticos descriptivos, distribuciones, patrones temporales y asociaciones entre variables. Sobre $D_{\mathrm{mod}}$, la identificación de patrones y la visualización exploratoria (series de tiempo, gráficos de dispersión y mapas temáticos) se integran en la Sección~\ref{sec:identificacion_patrones_tendencias}.

\subsection{Identificación de datos faltantes}
\label{sec:identificacion_datos_faltantes}
    Sobre el panel consolidado final (\texttt{data\_consolidada.csv}), con $N = 87{.}007{.}074$ observaciones (grano activo eléctrico $\times$ día), se realizó un diagnóstico sistemático de valores faltantes (\texttt{NA}) con dos enfoques complementarios: (i)~conteo de \texttt{NA} por variable y (ii)~conteo de observaciones del panel con al menos un \texttt{NA} por fila. El objetivo fue cuantificar la magnitud y localizar el origen de la incompletitud como insumo del EDA y para fundamentar las decisiones de imputación o exclusión en la Etapa~4.
    
    En el análisis por columna, para cada una de las $16$ variables del panel se calculó el número absoluto de \texttt{NA} y su porcentaje respecto a $N$. En el análisis por fila, se determinó cuántas observaciones $(i,t)$ presentan cero, uno o más campos incompletos, junto con el máximo de variables faltantes registradas en una misma fila.
    
    Los resultados muestran que la incompletitud se concentra en las variables climáticas integradas desde NASA POWER en el paso~\ref{paso:int-6}. Las seis variables meteorológicas (\texttt{velocidad\_viento\_ms}, \texttt{temperatura\_media\_c}, \texttt{humedad\_relativa\_pct}, \texttt{temperatura\_minima\_c}, \texttt{temperatura\_maxima\_c} y \texttt{precipitacion\_total\_mm}) presentan exactamente $3{.}401{.}838$ valores faltantes cada una, equivalente al $3{.}91$~\% de $N$. Este patrón es coherente con el cruce izquierdo por (\texttt{ELEMENTO}, \texttt{FECHA\_DIA}): las filas sin emparejamiento en la base climática conservan \texttt{NA} en el bloque meteorológico. La variable \texttt{ALTITUD} registra $32{.}868$ faltantes ($0{.}04$~\% de $N$), asociados al inventario GIS. El resto de variables del panel, identificación del activo (\texttt{ELEMENTO}, \texttt{TIPO}, \texttt{LATITUD}, \texttt{LONGITUD}), calendario (\texttt{FECHA\_DIA}), severidad (\texttt{CANTIDAD\_ELEMENTOS}, \texttt{CANTIDAD\_USUARIOS}) y continuidad (\texttt{DURACION\_min}, \texttt{FRECUENCIA}), no presentan \texttt{NA}, dado que los pasos~\ref{paso:int-5} y~\ref{paso:int-7} asignan $0$ en días sin evento.
    
    A nivel de fila, $83{.}576{.}020$ observaciones ($96{.}06$~\%) están completas; $3{.}431{.}054$ filas ($3{.}94$~\%) tienen al menos un \texttt{NA}, y el máximo de variables incompletas en una misma observación es $7$ (combinación de las seis variables climáticas más \texttt{ALTITUD}). La Figura~\ref{fig:faltantes_por_variable} y la Tabla~\ref{tab:resumen_datos_faltantes} resumen el porcentaje de faltantes por variable, ordenado de mayor a menor.
    
    \begin{figure}[!htbp]
\centering
        \includegraphics[width=0.95\textwidth]{img/grafico_datos_faltantes_por_variable.png}
        \caption{Porcentaje de valores faltantes por variable en el panel consolidado ($N = 87{.}007{.}074$).}
        \label{fig:faltantes_por_variable}
    \end{figure}
    
    \begin{table}[!htbp]
    \centering
\small
    \caption{Resumen de valores faltantes por variable en el panel consolidado}
    \label{tab:resumen_datos_faltantes}
    \begin{tabular}{l r r}
\toprule
    \textbf{Variable} & \textbf{$n$ de \texttt{NA}} & \textbf{\% sobre $N$} \\
\midrule
    \texttt{velocidad\_viento\_ms} & 3{.}401{.}838 & 3.91 \\
    \texttt{temperatura\_media\_c} & 3{.}401{.}838 & 3.91 \\
    \texttt{humedad\_relativa\_pct} & 3{.}401{.}838 & 3.91 \\
    \texttt{temperatura\_minima\_c} & 3{.}401{.}838 & 3.91 \\
    \texttt{temperatura\_maxima\_c} & 3{.}401{.}838 & 3.91 \\
    \texttt{precipitacion\_total\_mm} & 3{.}401{.}838 & 3.91 \\
    \texttt{ALTITUD} & 32{.}868 & 0.04 \\
    \midrule
    \multicolumn{3}{l}{\textit{Resto de variables del panel:} $0$ \texttt{NA} ($0$~\%)} \\
    \bottomrule
    \end{tabular}
    \end{table}
    
    \noindent En síntesis, la calidad estructural del panel es alta en variables operativas de interrupción (sin \texttt{NA}), y la incompletitud relevante para el modelado se limita al bloque climático y, en menor medida, a \texttt{ALTITUD}. Las reglas de imputación o exclusión se definen con base en este diagnóstico y se aplican en la Etapa~4.
    
    \paragraph{Tabla agrupada cuadrante $\times$ semana.} Sobre la tabla de análisis \texttt{data\_agrupada.csv} (Sección~\ref{sec:agrupacion_cuadrante_semana}), con $N_{\mathrm{agr}} = 90{.}128$ observaciones y $16$ variables, se replicó el mismo esquema de diagnóstico: conteo de \texttt{NA} por variable y por fila. El procedimiento se documenta en el Apéndice~\ref{app:html-faltantes-agrupada}.
    
    En el análisis por columna, las $16$ variables del grano agregado, identificación y calendario (\texttt{id\_cuadrante}, \texttt{semana}), georreferenciación (\texttt{latitud}, \texttt{longitud}, \texttt{altitud}), severidad (\texttt{usuarios\_afectados}, \texttt{elementos\_afectados}), clima semanal (\texttt{temp\_media\_semana}, \texttt{temp\_max\_semana}, \texttt{temp\_min\_semana}, \texttt{humedad\_media\_semana}, \texttt{viento\_medio\_semana}, \texttt{viento\_max\_semana}, \texttt{precip\_total\_semana}) y continuidad (\texttt{n\_eventos}, \texttt{duracion\_total}), registran $0$ valores faltantes ($0$~\% sobre $N_{\mathrm{agr}}$). A nivel de fila, las $90{.}128$ observaciones están completas: ninguna fila presenta al menos un \texttt{NA} y el máximo de variables incompletas por fila es $0$.
    
    Este resultado es coherente con la agregación descrita en la Sección~\ref{sec:agrupacion_cuadrante_semana}: las sumas y promedios semanales emplean \texttt{na.rm = TRUE}, de modo que los \texttt{NA} del panel diario no se propagan como campos vacíos en el grano agregado; las métricas operativas sin evento se consolidan en $0$, y cada fila exportada conserva llaves estructurales definidas (\texttt{id\_cuadrante}, \texttt{semana}). La Figura~\ref{fig:heatmap_faltantes_agrupada} ilustra la ausencia de faltantes en una muestra de las primeras $50$ filas: todas las celdas se clasifican como completas.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/heatmap_datos_faltantes_agrupada.png}
        \caption{Completitud de la tabla agrupada por fila y variable ($N_{\mathrm{agr}} = 90{.}128$; muestra de $50$ filas).}
        \label{fig:heatmap_faltantes_agrupada}
    \end{figure}
    
    \noindent En consecuencia, la tabla agrupada no requiere reglas de imputación por valores faltantes en la Etapa~4; el tratamiento de incompletitud documentado en los párrafos anteriores aplica al panel consolidado activo~$\times$~día, grano en el que se originan los \texttt{NA} climáticos y de \texttt{ALTITUD}.
    
\subsection{Identificación de valores extremos en variables cuantitativas}
\label{sec:identificacion_valores_extremos}
    Se evaluó la presencia de valores extremos en las variables cuantitativas mediante reglas univariadas y diagramas de caja e histogramas. El análisis distingue observaciones operativamente plausibles de posibles inconsistencias de registro, sin aplicar aún transformaciones ni exclusiones definitivas; el tratamiento se difiere a la Etapa~4 con base en estos hallazgos. Para cada variable se aplicaron la regla de Tukey~\textcite{ref50}, el rango intercuartílico ampliado y el criterio de percentiles P1--P99, conforme a lo formalizado en la Sección~\ref{sec:metodos-modelos}. Como complemento interpretativo de la dispersión relativa entre variables de distinta escala, se reporta el coeficiente de variación (CV; Sección~\ref{sec:metodos-modelos}).
    
    \paragraph{Panel consolidado activo $\times$ día.} Sobre \texttt{data\_consolidada.csv} ($N = 87{.}007{.}074$) se aplicó el protocolo descrito, excluyendo coordenadas geográficas (\texttt{LATITUD}, \texttt{LONGITUD}, \texttt{ALTITUD}). El detalle reproducible se documenta en el Apéndice~\ref{app:html-outliers}.
    
    \subparagraph{Variables climáticas diarias.} Las variables de temperatura y humedad presentan distribuciones aproximadamente simétricas, baja dispersión relativa (CV entre $0{.}16$ y $0{.}20$) y un porcentaje muy bajo de extremos bajo la regla de Tukey (entre $0$~\% para \texttt{humedad\_relativa\_pct} y $0{.}20$~\% para \texttt{temperatura\_minima\_c}); con el factor $3{.}0$ prácticamente no se identifican extremos severos. El viento exhibe mayor heterogeneidad relativa (CV $= 0{.}69$). La \texttt{precipitacion\_total\_mm} constituye la excepción: por su naturaleza fuertemente asimétrica, con exceso de ceros y CV $= 2{.}12$, la regla de Tukey marca el $10{.}38$~\% de las observaciones y la ampliada el $5{.}30$~\%, aunque la regla de percentiles solo señala el $1{.}00$~\%. Estos valores corresponden a días de lluvia intensa operativamente plausibles, no a errores de registro. La Tabla~\ref{tab:extremos_clima} resume estos resultados y las Figuras~\ref{fig:boxplot_clima} y~\ref{fig:histograma_clima} muestran, respectivamente, los diagramas de caja y los histogramas sobre una muestra de $200{.}000$ observaciones.

\begin{table}[!htbp]
\centering
\small
    \caption{Valores extremos y dispersión relativa en variables climáticas (panel completo)}
    \label{tab:extremos_clima}
    \begin{tabular}{l r r r r r}
\toprule
    \textbf{Variable} & \textbf{Mediana} & \textbf{CV} & \textbf{\% IQR 1.5} & \textbf{\% IQR 3.0} & \textbf{\% P1--P99} \\
\midrule
    \texttt{velocidad\_viento\_ms}   & 1.20  & 0.69 & 0.03  & 0.00 & 1.60 \\
    \texttt{temperatura\_media\_c}   & 25.47 & 0.17 & 0.02  & 0.00 & 1.99 \\
    \texttt{humedad\_relativa\_pct}  & 69.08 & 0.18 & 0.00  & 0.00 & 1.98 \\
    \texttt{temperatura\_minima\_c}  & 20.91 & 0.20 & 0.20  & 0.00 & 1.99 \\
    \texttt{temperatura\_maxima\_c}  & 31.32 & 0.16 & 0.05  & 0.00 & 1.99 \\
    \texttt{precipitacion\_total\_mm} & 0.50 & 2.12 & 10.38 & 5.30 & 1.00 \\
    \bottomrule
    \end{tabular}
    \end{table}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/boxplot_valores_extremos_clima.png}
        \caption{Distribución de las variables climáticas: mediana y extremos (muestra de $200{.}000$ observaciones).}
        \label{fig:boxplot_clima}
    \end{figure}

    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/histograma_valores_extremos_clima.png}
        \caption{Distribución de las variables climáticas: forma y cola (muestra de $200{.}000$ observaciones).}
        \label{fig:histograma_clima}
    \end{figure}
    
    \subparagraph{Variables de severidad y continuidad diarias.} Estas variables están dominadas por ceros (días sin interrupción): el $99{.}94$~\% de las filas registra $0$, por lo que sobre el panel completo se cumple $Q_1 = Q_3 = 0$ y la regla de Tukey resulta poco informativa (marca como extremo cualquier día con evento, $0{.}06$~\% del panel); el CV del panel completo queda artificialmente inflado por la masa en cero. Por ello, el análisis relevante se realiza sobre los días con evento ($\text{valor} > 0$), donde se concentran $48{.}375$ registros de afectación y $48{.}364$ de continuidad. En este subconjunto las distribuciones son fuertemente asimétricas a la derecha y de alta dispersión relativa: la mediana de \texttt{CANTIDAD\_USUARIOS} es $23$ usuarios pero su máximo alcanza $34{.}622$ (CV $= 3{.}97$); la \texttt{DURACION\_min} tiene mediana de $417$ minutos ($\approx 7$~horas) y un máximo de $332{.}395$ minutos ($\approx 231$~días; CV $= 3{.}56$), sugerente de posibles inconsistencias de registro que se marcan para revisión. La regla de Tukey marca entre el $9$~\% y el $15$~\% de los días con evento como extremos, proporción esperable en distribuciones de cola pesada. La Tabla~\ref{tab:extremos_evento} resume estos resultados y las Figuras~\ref{fig:boxplot_evento} y~\ref{fig:histograma_evento} presentan los diagramas de caja e histogramas sobre los días con evento.
    
    \begin{table}[!htbp]
    \centering
    \small
    \caption{Valores extremos y dispersión relativa en severidad y continuidad (días con evento, $\text{valor} > 0$)}
    \label{tab:extremos_evento}
    \begin{tabular}{l r r r r r r}
    \toprule
    \textbf{Variable} & \textbf{$n$} & \textbf{Mediana} & \textbf{Máximo} & \textbf{CV} & \textbf{\% IQR 1.5} & \textbf{\% IQR 3.0} \\
    \midrule
    \texttt{CANTIDAD\_ELEMENTOS} & 48{.}375 & 2   & 2{.}469   & 3.15 & 14.91 & 11.78 \\
    \texttt{CANTIDAD\_USUARIOS}  & 48{.}375 & 23  & 34{.}622  & 3.97 & 14.50 & 10.83 \\
    \texttt{DURACION\_min}  & 48{.}364 & 417 & 332{.}395 & 3.56 & 8.94  & 5.36  \\
    \texttt{FRECUENCIA}          & 48{.}364 & 1   & 6         & 0.15 & 1.96  & 1.96  \\
\bottomrule
    \end{tabular}
    \end{table}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/boxplot_valores_extremos_evento.png}
        \caption{Distribución de la severidad y continuidad en días con evento: mediana y extremos.}
        \label{fig:boxplot_evento}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/histograma_valores_extremos_evento.png}
        \caption{Distribución de la severidad y continuidad en días con evento: forma y cola.}
        \label{fig:histograma_evento}
    \end{figure}
    
    \paragraph{Tabla agrupada cuadrante $\times$ semana.} Sobre \texttt{data\_agrupada.csv} (Sección~\ref{sec:agrupacion_cuadrante_semana}), con $N_{\mathrm{agr}} = 90{.}128$ observaciones, se replicó el mismo esquema excluyendo \texttt{id\_cuadrante}, \texttt{latitud}, \texttt{longitud} y \texttt{altitud}. El procedimiento se documenta en el Apéndice~\ref{app:html-outliers-agrupada}.
    
    \subparagraph{Variables climáticas semanales.} El diagnóstico se realizó sobre la tabla completa. Las variables de temperatura y humedad presentan distribuciones aproximadamente simétricas, baja dispersión relativa (CV entre $0{.}16$ y $0{.}25$) y un porcentaje muy bajo de extremos bajo Tukey (entre $0$~\% y $0{.}03$~\%); con el factor $3{.}0$ no se identifican extremos severos. El viento semanal es más heterogéneo (CV $\approx 0{.}78$--$0{.}81$). La \texttt{precip\_total\_semana} constituye la excepción: al ser un acumulado semanal asimétrico a la derecha con CV $= 1{.}17$, la regla de Tukey marca el $6{.}21$~\% de las observaciones y la ampliada el $1{.}79$~\%, aunque la regla de percentiles solo señala el $0{.}99$~\%. Los valores señalados corresponden a semanas de lluvia intensa operativamente plausibles: el máximo observado es de $198{.}1$~mm acumulados en la semana, magnitud compatible con los registros de temporada de lluvia en la región. La Tabla~\ref{tab:extremos_clima_agrupada} resume estos resultados y las Figuras~\ref{fig:boxplot_clima_agrupada} y~\ref{fig:histograma_clima_agrupada} muestran los diagramas de caja y los histogramas sobre las $90{.}128$ observaciones.
    
    \begin{table}[!htbp]
    \centering
    \small
    \caption{Valores extremos y dispersión relativa en variables climáticas semanales (tabla agrupada)}
    \label{tab:extremos_clima_agrupada}
    \begin{tabular}{l r r r r r}
    \toprule
    \textbf{Variable} & \textbf{Mediana} & \textbf{CV} & \textbf{\% IQR 1.5} & \textbf{\% IQR 3.0} & \textbf{\% P1--P99} \\
    \midrule
    \texttt{temp\_media\_semana}    & 24.84 & 0.19 & 0.00 & 0.00 & 1.95 \\
    \texttt{temp\_max\_semana}      & 32.72 & 0.17 & 0.00 & 0.00 & 1.99 \\
    \texttt{temp\_min\_semana}      & 18.95 & 0.25 & 0.03 & 0.00 & 1.96 \\
    \texttt{humedad\_media\_semana} & 72.28 & 0.16 & 0.00 & 0.00 & 1.97 \\
    \texttt{viento\_medio\_semana}  & 0.91  & 0.81 & 0.00 & 0.00 & 1.99 \\
    \texttt{viento\_max\_semana}    & 1.26  & 0.78 & 0.00 & 0.00 & 1.69 \\
    \texttt{precip\_total\_semana}  & 9.78  & 1.17 & 6.21 & 1.79 & 0.99 \\
    \bottomrule
    \end{tabular}
\end{table}

    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/boxplot_valores_extremos_clima_agrupada.png}
        \caption{Distribución de las variables climáticas semanales: mediana y extremos ($N_{\mathrm{agr}} = 90{.}128$).}
        \label{fig:boxplot_clima_agrupada}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/histograma_valores_extremos_clima_agrupada.png}
        \caption{Distribución de las variables climáticas semanales: forma y cola ($N_{\mathrm{agr}} = 90{.}128$).}
        \label{fig:histograma_clima_agrupada}
    \end{figure}
    
    \subparagraph{Variables de severidad y continuidad.} En la tabla completa persisten semanas sin interrupción registrada ($Q_1 = Q_3 = 0$ en \texttt{usuarios\_afectados}, \texttt{elementos\_afectados} y \texttt{n\_eventos}), por lo que la regla de Tukey sobre el total es poco informativa para esas variables y el CV del panel completo queda inflado por ceros (Tabla~\ref{tab:desc_evento_agrupada}). El análisis relevante se realiza sobre semanas con evento ($\text{valor} > 0$), con $22{.}660$--$24{.}800$ registros según la variable. En este subconjunto las distribuciones son fuertemente asimétricas a la derecha y de alta dispersión relativa: la mediana de \texttt{usuarios\_afectados} es $52$ usuarios pero su máximo alcanza $69{.}928$ (CV $= 3{.}82$); \texttt{duracion\_total} tiene mediana de $19{.}7$~horas y un máximo de $2{.}227$~horas ($\approx 93$~días acumulados; CV $= 1{.}73$). La regla de Tukey marca entre el $11{.}6$~\% y el $15{.}2$~\% de las semanas con evento como extremos, proporción esperable en distribuciones de cola pesada. La Tabla~\ref{tab:extremos_evento_agrupada} resume estos resultados y las Figuras~\ref{fig:boxplot_evento_agrupada} y~\ref{fig:histograma_evento_agrupada} presentan los diagramas de caja e histogramas.

\begin{table}[!htbp]
\centering
    \small
    \caption{Valores extremos y dispersión relativa en severidad y continuidad (semanas con evento, $\text{valor} > 0$)}
    \label{tab:extremos_evento_agrupada}
    \begin{tabular}{l r r r r r r}
    \toprule
    \textbf{Variable} & \textbf{$n$} & \textbf{Mediana} & \textbf{Máximo} & \textbf{CV} & \textbf{\% IQR 1.5} & \textbf{\% IQR 3.0} \\
    \midrule
    \texttt{usuarios\_afectados}  & 22{.}660 & 52    & 69{.}928 & 3.82 & 15.16 & 11.21 \\
    \texttt{elementos\_afectados} & 22{.}660 & 7     & 3{.}626  & 2.75 & 15.00 & 10.70 \\
    \texttt{duracion\_total}      & 24{.}800 & 19.7  & 2{.}227  & 1.73 & 11.56 & 4.25  \\
    \texttt{n\_eventos}            & 22{.}659 & 1     & 58       & 1.08 & 14.53 & 5.73  \\
    \bottomrule
    \end{tabular}
    \end{table}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/boxplot_valores_extremos_evento_agrupada.png}
        \caption{Distribución de la severidad y continuidad en semanas con evento: mediana y extremos.}
        \label{fig:boxplot_evento_agrupada}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/histograma_valores_extremos_evento_agrupada.png}
        \caption{Distribución de la severidad y continuidad en semanas con evento: forma y cola.}
        \label{fig:histograma_evento_agrupada}
    \end{figure}
    
    \noindent En síntesis, en el grano cuadrante~$\times$~semana los extremos climáticos son escasos y operativamente plausibles (con la precipitación acumulada como única variable de asimetría apreciable y CV superior a la unidad), mientras que en severidad y continuidad la cola pesada y el CV condicionado a semanas con evento reflejan semanas de alta afectación o múltiples eventos en un mismo cuadrante. No se aplican transformaciones ni exclusiones en esta etapa; las decisiones de tratamiento se documentan y ejecutan en la Etapa~4 a partir de este diagnóstico sobre la tabla de análisis.
    
\subsection{Cálculo de estadísticos descriptivos}
\label{sec:estadisticos_descriptivos}
    Tras el diagnóstico de completitud (Sección~\ref{sec:identificacion_datos_faltantes}) y el análisis de valores extremos (Sección~\ref{sec:identificacion_valores_extremos}), se calculó un conjunto sistemático de estadísticos descriptivos sobre la tabla de análisis \texttt{data\_agrupada.csv} (Sección~\ref{sec:agrupacion_cuadrante_semana}), con $N_{\mathrm{agr}} = 90{.}128$ observaciones cuadrante~$\times$~semana. Se excluyeron las llaves y coordenadas (\texttt{id\_cuadrante}, \texttt{latitud}, \texttt{longitud}, \texttt{altitud}) y se evaluaron las $11$ variables numéricas de severidad, clima semanal y continuidad. El procedimiento reproducible se documenta en el Apéndice~\ref{app:html-descriptivos}.
    
    Para cada variable se reportaron: (i)~medidas de tendencia central (media, mediana y moda); (ii)~medidas de dispersión (rango, varianza, desviación estándar y coeficiente de variación; Sección~\ref{sec:metodos-modelos}); (iii)~medidas de posición (cuartiles, deciles y percentiles P1, P5, P95 y P99); (iv)~medidas de forma (asimetría y curtosis con exceso); y (v)~distribución de frecuencias de la categoría derivada \emph{actividad semanal del cuadrante}, construida a partir de \texttt{n\_eventos} (sin eventos; $1$ evento; $2$--$5$ eventos; más de $5$ eventos). Las Figuras~\ref{fig:histograma_descriptivo_agrupada} y~\ref{fig:boxplot_descriptivo_agrupada} presentan la vista panorámica de las $11$ variables mediante histogramas y diagramas de caja.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/histograma_descriptivo_agrupada.png}
        \caption{Distribución de las variables numéricas de la tabla agrupada: forma y cola ($N_{\mathrm{agr}} = 90{.}128$).}
        \label{fig:histograma_descriptivo_agrupada}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/boxplot_descriptivo_agrupada.png}
        \caption{Distribución de las variables numéricas de la tabla agrupada: mediana y extremos ($N_{\mathrm{agr}} = 90{.}128$).}
        \label{fig:boxplot_descriptivo_agrupada}
    \end{figure}
    
    \paragraph{Variables climáticas semanales.} Las siete variables meteorológicas agregadas presentan distribuciones aproximadamente simétricas y baja dispersión relativa: el coeficiente de variación (CV) se ubica entre $0{.}16$ (\texttt{humedad\_media\_semana}) y $0{.}81$ (\texttt{viento\_medio\_semana}). La temperatura media semanal tiene mediana de $24{.}8$~\textdegree C (media $23{.}8$~\textdegree C; $Q_1$--$Q_3$: $20{.}4$--$27{.}2$~\textdegree C) y asimetría cercana a cero ($-0{.}37$). La humedad relativa media semanal concentra la mediana en $72{.}3$~\% (CV $= 0{.}16$). La excepción es \texttt{precip\_total\_semana}: acumulado semanal con mediana de $9{.}8$~mm, media de $16{.}0$~mm, CV $= 1{.}17$ y asimetría positiva ($1{.}98$), con un recorrido de $0$ a $198{.}1$~mm y un percentil $99$ de $85{.}1$~mm que corresponde a semanas de lluvia intensa puntuales. La Tabla~\ref{tab:desc_clima_agrupada} resume tendencia central y dispersión; la Figura~\ref{fig:densidad_clima_agrupada} muestra las curvas de densidad.
    
    \begin{table}[!htbp]
    \centering
\small
    \caption{Estadísticos descriptivos de variables climáticas semanales (tabla agrupada)}
    \label{tab:desc_clima_agrupada}
    \begin{tabular}{l r r r r r}
    \toprule
    \textbf{Variable} & \textbf{Media} & \textbf{Mediana} & \textbf{Desv. est.} & \textbf{CV} & \textbf{Asimetría} \\
    \midrule
    \texttt{temp\_media\_semana}    & 23.8  & 24.8 & 4.63  & 0.19 & $-0{.}37$ \\
    \texttt{temp\_max\_semana}      & 31.9  & 32.7 & 5.29  & 0.17 & $-0{.}44$ \\
    \texttt{temp\_min\_semana}      & 17.9  & 19.0 & 4.49  & 0.25 & $-0{.}49$ \\
    \texttt{humedad\_media\_semana} & 70.9  & 72.3 & 11.5  & 0.16 & $-0{.}36$ \\
    \texttt{viento\_medio\_semana}  & 0.99  & 0.91 & 0.80  & 0.81 & 0.44 \\
    \texttt{viento\_max\_semana}    & 1.30  & 1.26 & 1.01  & 0.78 & 0.37 \\
    \texttt{precip\_total\_semana}  & 16.0  & 9.8  & 18.7  & 1.17 & 1.98 \\
    \bottomrule
    \end{tabular}
    \end{table}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/curvas_densidad_clima_agrupada.png}
        \caption{Forma de la distribución de las variables climáticas semanales ($N_{\mathrm{agr}} = 90{.}128$).}
        \label{fig:densidad_clima_agrupada}
    \end{figure}
    
    \paragraph{Variables de severidad y continuidad.} Las cuatro variables operativas están dominadas por ceros: la mediana de \texttt{usuarios\_afectados}, \texttt{elementos\_afectados}, \texttt{n\_eventos} y \texttt{duracion\_total} es $0$ en el $74{.}9$~\% de las semanas-cuadrante sin eventos registrados (Tabla~\ref{tab:frecuencias_actividad_agrupada}; Figuras~\ref{fig:barras_actividad_agrupada} y~\ref{fig:pastel_actividad_agrupada}). Cuando ocurre al menos un evento, las distribuciones son fuertemente leptocúrticas y con cola derecha pesada: \texttt{usuarios\_afectados} alcanza un máximo de $69{.}928$ usuarios (media $156{.}1$; CV del panel completo $= 7{.}81$, inflado por ceros; CV condicionado a semanas con evento $= 3{.}82$); \texttt{duracion\_total} un máximo de $2{.}227$~horas acumuladas en la semana (media $13{.}7$~h; CV completo $= 3{.}67$; CV condicionado $= 1{.}73$). La Tabla~\ref{tab:desc_evento_agrupada} resume las medidas principales del panel completo; la Tabla~\ref{tab:extremos_evento_agrupada} reporta el CV condicionado; la Figura~\ref{fig:densidad_evento_agrupada} presenta las densidades en escala logarítmica desplazada (Sección~\ref{sec:metodos-modelos}) para semanas con evento.
    
    \begin{table}[!htbp]
    \centering
    \small
    \caption{Estadísticos descriptivos de severidad y continuidad (tabla agrupada; panel completo)}
    \label{tab:desc_evento_agrupada}
    \begin{tabular}{l r r r r r r}
\toprule
    \textbf{Variable} & \textbf{Media} & \textbf{Mediana} & \textbf{Máx.} & \textbf{CV} & \textbf{Asimetría} & \textbf{$Q_3$} \\
\midrule
    \texttt{usuarios\_afectados}  & 156.1 & 0 & 69{.}928 & 7.81 & 20.6 & 1 \\
    \texttt{elementos\_afectados} & 12.0  & 0 & 3{.}626  & 5.74 & 14.1 & 1 \\
    \texttt{n\_eventos}            & 0.55  & 0 & 58       & 2.76 & 8.18 & 1 \\
    \texttt{duracion\_total}      & 13.7  & 0 & 2{.}227  & 3.67 & 10.2 & 2.5 \\
    \bottomrule
    \end{tabular}
    
    \noindent\textit{Nota:} el CV del panel completo queda inflado por la cero-inflación; el CV condicionado a semanas con evento se reporta en la Tabla~\ref{tab:extremos_evento_agrupada}.
    \end{table}
    
    \begin{table}[!htbp]
    \centering
    \small
    \caption{Distribución de frecuencias por actividad semanal del cuadrante}
    \label{tab:frecuencias_actividad_agrupada}
    \begin{tabular}{l r r r}
    \toprule
    \textbf{Categoría (\texttt{n\_eventos})} & \textbf{Frec. absoluta} & \textbf{Frec. relativa (\%)} & \textbf{Frec. acumulada (\%)} \\
    \midrule
    Sin eventos           & 67{.}469 & 74.86 & 74.86 \\
    $1$ evento            & 12{.}115 & 13.44 & 88.30 \\
    $2$ a $5$ eventos     &  9{.}246 & 10.26 & 98.56 \\
    Más de $5$ eventos    &  1{.}298 &  1.44 & 100.00 \\
\bottomrule
    \end{tabular}
    \end{table}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.85\textwidth]{img/grafico_barras_actividad_agrupada.png}
        \caption{Frecuencia de semanas-cuadrante según actividad de eventos ($N_{\mathrm{agr}} = 90{.}128$).}
        \label{fig:barras_actividad_agrupada}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.70\textwidth]{img/grafico_pastel_actividad_agrupada.png}
        \caption{Distribución porcentual de semanas-cuadrante por actividad de eventos.}
        \label{fig:pastel_actividad_agrupada}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/curvas_densidad_evento_agrupada.png}
        \caption{Distribución de la severidad y continuidad en escala logarítmica desplazada (semanas con evento).}
        \label{fig:densidad_evento_agrupada}
    \end{figure}
    
    \noindent En síntesis, el clima semanal exhibe comportamiento estable y casi simétrico, mientras que las variables operativas reflejan fuerte cero-inflación y colas pesadas propias del fenómeno de interrupciones. Estos perfiles, especialmente la asimetría de \texttt{precip\_total\_semana} y la concentración de semanas sin evento, orientan las transformaciones logarítmicas desplazadas (Sección~\ref{sec:metodos-modelos}) y la selección de modelos en la Etapa~4.

\subsection{Análisis de la distribución de variables}
\label{sec:distribucion_variables}
    Con el propósito de comprender el comportamiento estadístico de las variables incluidas en el estudio, se realizó un análisis de distribución sobre las variables climáticas, de severidad y de continuidad contenidas en la tabla agregada cuadrante~$\times$~semana ($N_{\mathrm{agr}} = 90{.}128$; Sección~\ref{sec:agrupacion_cuadrante_semana}). Para las variables cuantitativas se emplearon diagramas de densidad, complementados con indicadores de forma (asimetría, curtosis y proporción de ceros) y con el coeficiente de variación reportado en las Secciones~\ref{sec:identificacion_valores_extremos}--\ref{sec:estadisticos_descriptivos}, lo que permitió evaluar simetría, dispersión relativa, unimodalidad y presencia de valores extremos. El procedimiento reproducible se documenta en el Apéndice~\ref{app:html-distribucion}.
    
    \paragraph{Variables climáticas.} Los resultados evidencian que las variables meteorológicas presentan distribuciones heterogéneas. Las temperaturas (media, máxima y mínima) y la humedad relativa muestran comportamientos estables, aproximadamente unimodales, con asimetría cercana a cero (entre $-0{.}49$ y $-0{.}36$) y CV bajo ($0{.}16$--$0{.}25$), coherentes con las curvas de densidad presentadas en la Figura~\ref{fig:densidad_clima_agrupada}. En contraste, la precipitación acumulada semanal presenta asimetría positiva (coeficiente de $1{.}98$ y curtosis con exceso de $4{.}78$), CV $= 1{.}17$, alta frecuencia de semanas con valores bajos de lluvia (el $9{.}1$~\% registra $0$~mm) y un número reducido de observaciones con precipitaciones intensas. La Figura~\ref{fig:densidad_precipitacion_agrupada} contrasta la distribución en escala original y en escala logarítmica desplazada (Sección~\ref{sec:metodos-modelos}): la transformación aproxima la distribución a una forma simétrica, lo que anticipa su utilidad en la Etapa~4.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/distribucion_densidad_precipitacion_agrupada.png}
        \caption{Distribución de la precipitación acumulada semanal en escala original y logarítmica desplazada ($N_{\mathrm{agr}} = 90{.}128$).}
        \label{fig:densidad_precipitacion_agrupada}
    \end{figure}
    
    \paragraph{Variables de severidad y continuidad.} Las variables asociadas a interrupciones del servicio (\texttt{usuarios\_afectados}, \texttt{elementos\_afectados}, \texttt{duracion\_total} y \texttt{n\_eventos}) muestran distribuciones altamente sesgadas hacia cero: la proporción de filas con valor $0$ se ubica entre el $72{.}5$~\% (\texttt{duracion\_total}) y el $74{.}9$~\% (resto de variables), como ilustra la Figura~\ref{fig:pct_ceros_evento_agrupada}. Este comportamiento es esperado, dado que la mayor parte de las semanas-cuadrante no registra fallas. Sin embargo, condicionando a la ocurrencia de eventos ($\text{valor} > 0$), las distribuciones presentan colas derechas largas, con asimetrías entre $8{.}2$ y $20{.}6$ y curtosis entre $139$ y $643$, que reflejan la existencia de incidentes de alta severidad con impacto significativo sobre la continuidad del servicio, según se aprecia en las densidades en escala logarítmica desplazada de la Figura~\ref{fig:densidad_evento_agrupada}.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.85\textwidth]{img/distribucion_pct_ceros_evento_agrupada.png}
        \caption{Proporción de ceros en las variables de severidad y continuidad de la tabla agrupada.}
        \label{fig:pct_ceros_evento_agrupada}
    \end{figure}
    
    \noindent En síntesis, el análisis de distribución confirma dos regímenes claramente diferenciados: un bloque climático estable y casi simétrico (CV bajo, con la precipitación como excepción asimétrica de CV algo superior a la unidad) y un bloque operativo cero-inflado con colas pesadas y CV condicionado alto. Estos hallazgos fundamentan las transformaciones logarítmicas desplazadas (Sección~\ref{sec:metodos-modelos}) y la consideración de modelos aptos para datos cero-inflados en la Etapa~4.
    
\subsection{Análisis temporal de eventos}
\label{sec:analisis_temporal_eventos}
    Para caracterizar la dinámica temporal de las interrupciones se construyó, a partir de la tabla agregada \texttt{data\_agrupada.csv} (Sección~\ref{sec:agrupacion_cuadrante_semana}), la serie territorial semanal que consolida el total de \texttt{n\_eventos} de todos los cuadrantes en cada semana calendario. La serie cubre $262$ semanas del horizonte 2021--2025, con una mediana de $124{.}5$ eventos por semana (media $188{.}5$; máximo $648$). El procedimiento reproducible se documenta en el Apéndice~\ref{app:html-temporal}.
    
    \paragraph{Evolución de la serie semanal.} La Figura~\ref{fig:serie_semanal_eventos} presenta la serie semanal con su tendencia suavizada (LOESS) sobre el horizonte exploratorio 2021--2025 ($D_{\mathrm{EDA}}$). Se distinguen dos regímenes visuales: entre 2021 y 2023 la actividad se mantiene en un nivel bajo y relativamente estable, mientras que a partir de 2024 se observa un incremento sostenido del nivel de la serie. La agregación anual (Figura~\ref{fig:eventos_por_anio}) cuantifica este cambio: de $5{.}167$ eventos en 2021 y $4{.}934$ en 2022, se pasa a $15{.}211$ en 2024 y $18{.}199$ en 2025; en términos de intensidad, el promedio asciende de $\approx 99$ a $\approx 350$ eventos por semana entre 2021 y 2025. La reinterpretación de este salto a la luz de la cobertura de integración se desarrolla en el párrafo siguiente (Sección~\ref{sec:delimitacion_temporal_modelado}).
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/serie_semanal_eventos_agrupada.png}
        \caption{Evolución semanal de los eventos de interrupción (suma territorial), 2021--2025.}
        \label{fig:serie_semanal_eventos}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.80\textwidth]{img/eventos_por_anio_agrupada.png}
        \caption{Total anual de eventos de interrupción sobre la tabla agrupada.}
        \label{fig:eventos_por_anio}
    \end{figure}
    
    \paragraph{Diagnóstico de cobertura del emparejamiento evento--elemento.}
    \label{sec:delimitacion_temporal_modelado}
    
    Al contrastar el conteo anual de eventos en \texttt{tabla\_eventos\_ajustado.csv} (BRAE) con la suma territorial de \texttt{n\_eventos} en \texttt{data\_agrupada.csv}, se identificó un \textit{sesgo sistemático} en el grano espacial durante 2021--2023 (Tabla~\ref{tab:cobertura_emparejamiento_oms}; Figura~\ref{fig:cobertura_emparejamiento}). La causa no es la ausencia del campo \texttt{COMPONENTE\_FALLADO}, completo en el $100$~\% de los registros BRAE, sino la \textit{incompatibilidad entre esquemas de codificación} al cruzar eventos con el inventario georreferenciado \texttt{tipo\_elemento.csv} (Tabla~\ref{tab:muestra_inventario_gis}).
    
    En la operación de la red coexisten dos identificadores para un mismo punto de conexión: (i)~el \textit{código de sitio} (ubicación), estable en el tiempo y empleado por el inventario GIS como \texttt{ELEMENTO}; y (ii)~el \textit{código de activo}, asociado al equipo instalado y \textit{sujeto a cambio} cuando el activo se repone por mantenimiento o renovación, mientras el código de sitio permanece invariante. El panel consolidado y la agregación cuadrante~$\times$~semana se construyen sobre el código de sitio. No obstante, en el sistema histórico OMS-SPARD (Sección~\ref{sec:extraccion_oms_historico}) el elemento fallado (\texttt{BREAKER} $\rightarrow$ \texttt{COMPONENTE\_FALLADO}) se registraba con frecuencia mediante el \textit{código del activo} y no el del sitio. Ello impide el seguimiento histórico del punto de desconexión, unidad espacial central para el pronóstico territorial de $SAIDI$ y $SAIFI$ en esta tesis, pues el cruce \texttt{COMPONENTE\_FALLADO} $=$ \texttt{ELEMENTO} no encuentra coincidencia en el inventario y el evento no genera \texttt{FRECUENCIA} en el panel activo~$\times$~día.
    
    El diagnóstico cuantitativo confirma esta hipótesis: la proporción de eventos cuyo \texttt{COMPONENTE\_FALLADO} coincide con algún \texttt{ELEMENTO} de \texttt{tipo\_elemento.csv} es del $30{.}5$~\% (2021), $30{.}9$~\% (2022) y $31{.}7$~\% (2023), valores cercanos a la cobertura del panel espacial en esos años ($28{.}1$~\%, $28{.}1$~\% y $29{.}1$~\%, respectivamente). Con la migración a OMS-SP7 (Sección~\ref{sec:extraccion_oms_actual}), la coincidencia asciende al $79{.}8$~\% (2024) y al $95{.}0$~\% (2025), acompañada por coberturas del panel del $77{.}7$~\% y $92{.}4$~\%. La Figura~\ref{fig:eventos_por_anio} debe interpretarse a la luz de esta brecha: el salto a partir de 2024 refleja en gran medida una \textit{mayor homogeneización de la codificación} bajo OMS-SP7, y no únicamente un incremento operativo de interrupciones.
    
    El procedimiento reproducible se documenta en el Apéndice~\ref{app:html-emparejamiento}. Con base en este hallazgo, emergido en el presente análisis temporal, se define el \textit{dataset de modelado} $D_{\mathrm{mod}}$ como el subconjunto de \texttt{data\_agrupada.csv} con \texttt{year(semana)} $\in \{2024,\,2025\}$, que comprende $36{.}120$ filas cuadrante~$\times$~semana, $105$ semanas y $33{.}410$ eventos georreferenciados en total ($15{.}211$ en 2024 y $18{.}199$ en 2025). El horizonte 2021--2025 se conserva como $D_{\mathrm{EDA}}$ para las Secciones~\ref{sec:identificacion_datos_faltantes}--\ref{sec:analisis_temporal_eventos}, pero \textit{no} se empleará para estimar el modelo predictivo de la Etapa~4.
    
    \begin{table}[!htbp]
    \centering
\footnotesize
    \setlength{\tabcolsep}{4pt}
    \caption{Cobertura anual de la trazabilidad al inventario GIS y al panel espacial.}
    \label{tab:cobertura_emparejamiento_oms}
    \begin{tabular}{@{}l r r r r@{}}
    \toprule
    \textbf{Año} & \textbf{Eventos BRAE} & \textbf{En inventario (\%)} & \textbf{Panel espacial} & \textbf{Cobertura (\%)} \\
    \midrule
    2021 & 18{.}398 & 30.5 & 5{.}167  & 28.1 \\
    2022 & 17{.}542 & 30.9 & 4{.}934  & 28.1 \\
    2023 & 20{.}068 & 31.7 & 5{.}845  & 29.1 \\
    2024 & 19{.}583 & 79.8 & 15{.}211 & 77.7 \\
    2025 & 19{.}689 & 95.0 & 18{.}199 & 92.4 \\
    \bottomrule
    \end{tabular}
    
    \noindent\textit{Nota:} ``En inventario (\%)'' es la proporción de eventos cuyo \texttt{COMPONENTE\_FALLADO} coincide con \texttt{ELEMENTO} en \texttt{tipo\_elemento.csv} (código de sitio). ``Panel espacial'' es la suma anual de \texttt{n\_eventos} en \texttt{data\_agrupada.csv}. La brecha entre ambas columnas en 2021--2023 se atribuye al registro del código de activo, y no del código de sitio, en OMS-SPARD, lo que impide el seguimiento histórico del punto de desconexión. Los años 2024--2025 delimitan $D_{\mathrm{mod}}$.
\end{table}

    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/cobertura_emparejamiento_por_anio.png}
        \caption{Comparación anual entre eventos BRAE y eventos reflejados en el panel espacial. Las etiquetas indican el porcentaje de cobertura respecto a BRAE; la brecha histórica se explica por la incompatibilidad entre el código de activo registrado en OMS-SPARD y el código de sitio del inventario GIS.}
        \label{fig:cobertura_emparejamiento}
    \end{figure}
    
    \paragraph{Estacionalidad intra-anual.} La Figura~\ref{fig:estacionalidad_mensual} muestra la distribución de los totales semanales según el mes calendario. Se aprecia heterogeneidad estacional, con mayor actividad y dispersión en algunos meses del año, patrón que motiva incorporar términos estacionales en la Etapa~4. La atribución de esta estacionalidad a la temporada de lluvias debe tomarse con cautela: como se documenta en la Sección~\ref{sec:analisis_correlacion_asociacion}, la asociación semanal entre precipitación y frecuencia de eventos es débil en el grano cuadrante~$\times$~semana.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/estacionalidad_mensual_eventos_agrupada.png}
        \caption{Distribución del total semanal de eventos según mes calendario (2021--2025).}
        \label{fig:estacionalidad_mensual}
    \end{figure}
    
    \paragraph{Autocorrelación temporal.} Las funciones de autocorrelación simple y parcial de la serie semanal (Figura~\ref{fig:acf_serie_semanal}) evidencian fuerte persistencia: la autocorrelación es $0{.}92$ en el rezago de una semana y decae lentamente ($0{.}60$ a $12$ semanas; $0{.}38$ a $26$), manteniéndose significativa frente al intervalo de confianza del $95$~\% bajo ruido blanco ($\pm 0{.}12$). Adicionalmente, el rezago de $52$ semanas conserva una autocorrelación de $0{.}43$, indicativa de un componente estacional anual. Esta estructura justifica la incorporación de rezagos y términos estacionales como variables predictoras en la Etapa~4.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/acf_serie_semanal_agrupada.png}
        \caption{Persistencia y estructura temporal de la serie semanal de eventos (rezagos en semanas).}
        \label{fig:acf_serie_semanal}
    \end{figure}
    
    \paragraph{Evolución de la severidad y extensión territorial.} La Figura~\ref{fig:serie_severidad_semanal} presenta las series semanales de usuarios afectados y duración acumulada: ambas acompañan el incremento de nivel observado en la frecuencia de eventos a partir de 2024, lo que indica que el cambio de régimen no es solo de conteo sino también de impacto sobre la continuidad del servicio. Complementariamente, la Figura~\ref{fig:cuadrantes_activos_semana} muestra el número de cuadrantes con al menos un evento por semana, que crece en paralelo: la mayor actividad reciente se distribuye sobre una porción más amplia del territorio y no se explica por unos pocos cuadrantes puntuales.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/serie_severidad_semanal_agrupada.png}
        \caption{Evolución semanal de usuarios afectados y duración acumulada.}
        \label{fig:serie_severidad_semanal}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/cuadrantes_activos_semana_agrupada.png}
        \caption{Número de cuadrantes con al menos un evento por semana (extensión territorial de la actividad).}
        \label{fig:cuadrantes_activos_semana}
    \end{figure}
    
    \noindent En síntesis, el análisis temporal sobre $D_{\mathrm{EDA}}$ revela: (i)~un cambio de nivel de la serie a partir de 2024, reinterpretado como mayor homogeneización de la codificación activo/sitio bajo OMS-SP7, y no como incremento operativo aislado, lo que motiva la restricción del dataset de modelado a $D_{\mathrm{mod}}$ (2024--2025; Sección~\ref{sec:delimitacion_temporal_modelado}); (ii)~estacionalidad intra-anual en la actividad de interrupciones, cuya atribución al régimen de lluvias no se sostiene con el acumulado semanal de precipitación (Sección~\ref{sec:analisis_correlacion_asociacion}); y (iii)~fuerte persistencia semanal con componente anual, que fundamenta el uso de rezagos temporales como predictores en $D_{\mathrm{mod}}$. Estos hallazgos orientan la generación de ventanas temporales y la partición de datos de la Etapa~4.
    
    \paragraph{Materialización de $D_{\mathrm{mod}}$.} Para operacionalizar la delimitación temporal sin alterar el insumo exploratorio, se construyó el archivo \texttt{data\_agrupada\_mod.csv} como subconjunto de \texttt{data\_agrupada.csv} con filtro \texttt{year(semana)} $\in \{2024,\,2025\}$, conservando el grano cuadrante~$\times$~semana y el conjunto completo de variables climáticas y operativas. La tabla original 2021--2025 permanece intacta como $D_{\mathrm{EDA}}$; el resultado filtrado ($36{.}120$ filas; $344$ cuadrantes; $105$ semanas; $33{.}410$ eventos) constituye el insumo analítico para las Secciones~\ref{sec:identificacion_patrones_tendencias} en adelante y la Etapa~4, con partición natural entrenamiento 2024 / prueba 2025. El procedimiento reproducible, lectura, filtro, resumen por año y exportación, se documenta en el Apéndice~\ref{app:html-datamod}.

    \paragraph{Enriquecimiento de $D_{\mathrm{mod}}$.} Sobre la llave
    cuadrante--semana se incorporaron 16 variables destinadas a ampliar la
    caracterización de exposición, composición de usuarios y trayectoria de
    intervención de los activos. La serie mensual
    \texttt{data\_usuarios\_totales\_sistema.csv} aporta
    \texttt{usuarios\_totales}, asignada a cada semana de acuerdo con el mes
    calendario de su fecha de inicio y utilizada como denominador en la
    construcción de SAIDI y SAIFI. Por otra parte,
    \texttt{cant\_usuarios\_x\_transformador.csv} aporta
    \texttt{cantidad\_usuarios\_cuadrante}, calculada como la suma mensual de
    usuarios de los transformadores asociados a cada cuadrante.

    La composición socioeconómica y de uso del servicio se integró desde
    \texttt{DATA\_ESTRATO\_SECTOR.csv}, agregando por cuadrante y mes las
    variables \texttt{ESTRATO\_1} a \texttt{ESTRATO\_6},
    \texttt{INDUSTRIAL}, \texttt{COMERCIAL}, \texttt{OFICIAL},
    \texttt{PROVISIONAL} y \texttt{ALUMBRADO\_PUBLICO}. Para los meses
    faltantes de los cuadrantes 258 y 273 se empleó la moda mensual del
    respectivo cuadrante; los cuadrantes 68, 140, 333 y 344, ausentes en la
    fuente, se asignaron en cero.

    Finalmente, mediante la llave \texttt{ID\_CUADRANTE} se integró
    \texttt{Fecha\_transformadores.csv}. Para cada elemento se contabilizaron
    sus registros de modificación y se tomó la última
    \texttt{TT2\_FMODIFICACION} en estado \texttt{OPERACION}; posteriormente,
    por cuadrante, se derivaron \texttt{FECHA\_ACTIVO\_MAS\_VIEJO},
    \texttt{FECHA\_ACTIVO\_MAS\_NUEVO} y
    \texttt{PROM\_CANT\_MODIFICACIONES}. Estas variables permiten evaluar la
    relación entre la trayectoria de intervención de la infraestructura y los
    indicadores de continuidad. No obstante, las fechas representan
    modificaciones registradas y no necesariamente la instalación original
    del activo. Para evitar fuga de información, su uso predictivo debe
    restringirse a registros conocidos antes de la semana pronosticada;
    SAIDI y SAIFI se emplean exclusivamente como variables objetivo.

\subsection{Identificación de patrones, tendencias y visualización exploratoria}
\label{sec:identificacion_patrones_tendencias}
    Con fundamento en la delimitación de $D_{\mathrm{mod}}$ (Sección~\ref{sec:delimitacion_temporal_modelado}), se identifican patrones temporales, espaciales y de asociación clima--evento sobre la tabla \texttt{data\_agrupada\_mod.csv} ($36{.}120$ filas cuadrante~$\times$~semana; $344$ cuadrantes; $105$ semanas; $33{.}410$ eventos en 2024--2025). En esta misma subsección se integra la \textit{visualización exploratoria} sobre $D_{\mathrm{mod}}$: series de tiempo, diagramas de dispersión y mapas temáticos que complementan el análisis cuantitativo de patrones. El procedimiento reproducible se documenta en el Apéndice~\ref{app:html-patrones}.
    
    \paragraph{Patrones temporales.} La Figura~\ref{fig:serie_semanal_patrones_mod} presenta la serie semanal territorial de eventos en $D_{\mathrm{mod}}$, con media de $\approx 318$ eventos por semana. Aunque las etiquetas del eje horizontal se muestran cada 3 meses para facilitar la lectura, cada tramo de la linea corresponde a observaciones semanales consecutivas. La intensidad promedio asciende de $287$ eventos/semana en 2024 a $350$ en 2025. La estacionalidad mensual (Figura~\ref{fig:estacionalidad_patrones_mod}) confirma la heterogeneidad entre meses observada en la Sección~\ref{sec:analisis_temporal_eventos} sobre $D_{\mathrm{EDA}}$, sin que el acumulado semanal de lluvia explique ese patrón. La ACF de la serie semanal (Figura~\ref{fig:acf_patrones_mod}) mantiene persistencia elevada (rezago~$1 = 0{.}80$), lo que respalda el uso de rezagos como predictores en la Etapa~4.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/serie_semanal_patrones_mod.png}
        \caption{Evolución semanal de eventos sobre $D_{\mathrm{mod}}$ (2024--2025).}
        \label{fig:serie_semanal_patrones_mod}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/estacionalidad_mensual_patrones_mod.png}
        \caption{Estacionalidad mensual del total semanal de eventos en $D_{\mathrm{mod}}$.}
        \label{fig:estacionalidad_patrones_mod}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/acf_serie_patrones_mod.png}
        \caption{Persistencia y estructura temporal de la serie semanal de eventos en $D_{\mathrm{mod}}$.}
        \label{fig:acf_patrones_mod}
    \end{figure}
    
    \paragraph{Patrones espaciales.} El ranking de cuadrantes por total de \texttt{n\_eventos} (Figura~\ref{fig:ranking_cuadrantes_mod}) y el mapa de intensidad territorial (Figura~\ref{fig:mapa_intensidad_mod}) evidencian heterogeneidad espacial: los diez cuadrantes más activos concentran el $20{.}5$~\% de los eventos del periodo, mientras el $14{.}2$~\% de los cuadrantes ($49$ de $344$) se clasifica en nivel de actividad \emph{alta} (Figura~\ref{fig:segmentacion_mod}). La actividad no se distribuye de forma uniforme sobre la malla de $800$ cuadrantes, sino que se agrupa en zonas de mayor exposición operativa.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.85\textwidth]{img/ranking_cuadrantes_patrones_mod.png}
        \caption{Quince cuadrantes con mayor número acumulado de eventos en $D_{\mathrm{mod}}$.}
        \label{fig:ranking_cuadrantes_mod}
    \end{figure}
    
    \noindent La Figura~\ref{fig:ubicacion_top_cuadrantes_mod} complementa el ranking anterior con la \textit{ubicación geográfica} de los cuadrantes más activos sobre la malla territorial del área de estudio. Los recuadros resaltados corresponden a los primeros cuadrantes \emph{top} del periodo 2024--2025 y muestran una concentración en cuatro zonas: (i)~el área metropolitana de Cúcuta y Villa del Rosario; (ii)~un corredor al suroccidente, en torno a Salazar, Arboledas, Durania y Chinácota; (iii)~la subregión de Tibú, al norte del departamento; y (iv)~los municipios de Ocaña y Aguachica, al occidente. Esta distribución espacial confirma que la mayor actividad de interrupciones no es aleatoria, sino que se agrupa en núcleos urbanos y corredores de servicio de la red.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/ubicacion_top_cuadrantes_mod.png}
        \caption{Ubicación geográfica de los cuadrantes \emph{top} con mayor acumulado de eventos en $D_{\mathrm{mod}}$ (2024--2025). Los recuadros resaltados identifican la posición espacial de los primeros cuadrantes del ranking.}
        \label{fig:ubicacion_top_cuadrantes_mod}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.90\textwidth]{img/mapa_intensidad_cuadrantes_patrones_mod.png}
        \caption{Intensidad territorial de eventos por cuadrante (2024--2025).}
        \label{fig:mapa_intensidad_mod}
    \end{figure}
    
    \paragraph{Patrones clima--evento.} Mediante la estimación de la correlación de Spearman a nivel cuadrante~$\times$~semana, las variables de viento exhiben la asociación positiva más fuerte con \texttt{n\_eventos} (\texttt{viento\_max\_semana}: $\hat{\rho} = 0{.}14$; \texttt{viento\_medio\_semana}: $\hat{\rho} = 0{.}14$; $p < 0{.}001$), seguidas de la precipitación acumulada semanal (\texttt{precip total semana}: $\hat{\rho} = 0{.}07$) y la humedad ($\hat{\rho} = 0{.}05$). La Figura~\ref{fig:dispersion_precip_mod} ilustra la relación entre precipitación y frecuencia de eventos: la nube muestra una tendencia positiva leve, sin un patrón fuerte. Las temperaturas presentan asociaciones débiles y negativas ($|\hat{\rho}| < 0{.}05$). En el grano semanal, por tanto, la señal climática es de magnitud modesta y está encabezada por el viento, no por la lluvia.

    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.85\textwidth]{img/dispersion_precip_eventos_patrones_mod.png}
        \caption{Relación entre precipitación semanal y frecuencia de eventos por cuadrante en $D_{\mathrm{mod}}$.}
        \label{fig:dispersion_precip_mod}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.75\textwidth]{img/segmentacion_cuadrantes_patrones_mod.png}
        \caption{Segmentación de cuadrantes por nivel de actividad acumulada en $D_{\mathrm{mod}}$.}
        \label{fig:segmentacion_mod}
    \end{figure}
    
    \noindent En síntesis, el análisis de patrones y la visualización exploratoria sobre $D_{\mathrm{mod}}$ confirman: (i)~persistencia y estacionalidad temporal (series semanales, estacionalidad mensual y ACF); (ii)~concentración espacial en un subconjunto de cuadrantes (ranking, mapa de intensidad y segmentación por actividad); y (iii)~asociación positiva pero débil entre las covariables meteorológicas y la frecuencia de interrupciones, encabezada por el viento y seguida por la precipitación (correlación de Spearman y diagrama de dispersión). Estos hallazgos orientan la selección de variables predictoras y la estratificación territorial de la Etapa~4. Los mapas de calor de correlaciones y el análisis multivariado formal se desarrollan en las Secciones~\ref{sec:analisis_correlacion_asociacion} y~\ref{sec:evaluacion_multivariada}.
    
\subsection{Análisis de correlación y asociación}
\label{sec:analisis_correlacion_asociacion}
    Con fundamento en los patrones preliminares de la Sección~\ref{sec:identificacion_patrones_tendencias}, se realizó un análisis sistemático de asociación pareada sobre \texttt{data\_agrupada\_mod.csv} ($D_{\mathrm{mod}}$; $36{.}120$ filas cuadrante~$\times$~semana). Dado el carácter asimétrico, cero-inflado y de colas pesadas de las variables operativas (Sección~\ref{sec:distribucion_variables}), se empleó el coeficiente de correlación de \textit{Spearman} en lugar de Pearson. El procedimiento reproducible se documenta en el Apéndice~\ref{app:html-correlacion}.
    
    \paragraph{Matriz de asociación global.} Se calcularon las correlaciones de Spearman entre las variables operativas (\texttt{n\_eventos}, \texttt{usuarios\_afectados}, \texttt{elementos\_afectados}, \texttt{duracion\_total}), las siete variables climáticas semanales y las coordenadas geográficas (\texttt{altitud}, \texttt{latitud}, \texttt{longitud}). La Figura~\ref{fig:heatmap_cor_clima_operativo} resume la submatriz clima--operativo. Los pares con mayor asociación absoluta del panel completo concentran redundancia operativa: \texttt{usuarios\_afectados}--\texttt{elementos\_afectados} ($\rho = 0{.}99$), \texttt{n\_eventos}--\texttt{elementos\_afectados} ($\rho = 0{.}98$) y \texttt{n\_eventos}--\texttt{usuarios\_afectados} ($\rho = 0{.}97$). Parte de esta magnitud es \textit{mecánica}: las tres métricas provienen de la misma tabla de eventos, comparten la masa de ceros en semanas sin interrupción y, al agregarse por suma, crecen juntas cuando aumenta el número de eventos. Por ello no deben interpretarse como evidencia de tres dimensiones independientes ni usarse simultáneamente como respuesta y predictoras; la definición definitiva del rol de la severidad se aplaza a la Etapa~4.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.92\textwidth]{img/heatmap_correlacion_clima_operativo_mod.png}
        \caption{Asociación de Spearman entre variables climáticas y operativas en $D_{\mathrm{mod}}$.}
        \label{fig:heatmap_cor_clima_operativo}
    \end{figure}
    
    \paragraph{Bloque climático: redundancias.} En el bloque meteorológico (Figura~\ref{fig:heatmap_cor_clima}) se identificaron \textit{diez} pares con $|\rho| \geq 0{.}70$, principalmente entre temperaturas (\texttt{temp\_media}--\texttt{temp\_max}: $\rho = 0{.}96$; \texttt{temp\_media}--\texttt{temp\_min}: $\rho = 0{.}91$) y entre componentes de viento ($\rho = 0{.}97$). Las temperaturas presentan además asociación negativa moderada con la humedad ($\rho \approx -0{.}76$ a $-0{.}80$) y con el viento ($\rho \approx -0{.}70$ a $-0{.}81$). La precipitación no participa en ninguno de estos pares: su mayor asociación dentro del bloque queda por debajo del umbral, lo que la señala como la única covariable climática no redundante. Estos resultados anticipan \textit{multicolinealidad} entre temperaturas y vientos, y orientan la depuración de variables en la Etapa~4.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.85\textwidth]{img/heatmap_correlacion_clima_mod.png}
        \caption{Correlaciones de Spearman en el bloque climático semanal ($D_{\mathrm{mod}}$).}
        \label{fig:heatmap_cor_clima}
    \end{figure}
    
    \paragraph{Bloque operativo.} La Figura~\ref{fig:heatmap_cor_operativo} muestra que las cuatro métricas de continuidad y severidad están fuertemente correlacionadas entre sí ($\rho$ entre $0{.}83$ y $0{.}99$), aunque \texttt{duracion\_total} exhibe una asociación algo menor con \texttt{n\_eventos} ($\rho = 0{.}86$) que con las variables de afectación. Frecuencia y duración capturan dimensiones parcialmente distintas; el alcance (\texttt{usuarios\_afectados}, \texttt{elementos\_afectados}) queda, en esta construcción, muy alineado con la frecuencia y se trata aquí como diagnóstico exploratorio de severidad, no como predictor independiente.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.72\textwidth]{img/heatmap_correlacion_operativo_mod.png}
        \caption{Correlaciones de Spearman entre variables operativas en $D_{\mathrm{mod}}$.}
        \label{fig:heatmap_cor_operativo}
    \end{figure}
    
    \paragraph{Clima $\rightarrow$ operativo.} La Figura~\ref{fig:barras_cor_clima_operativo} y la Tabla~\ref{tab:clima_operativo_cor} cuantifican la asociación entre covariables meteorológicas y métricas de interrupción. Todas las asociaciones son estadísticamente significativas, consecuencia esperable del tamaño del panel, pero de magnitud pequeña. Las variables de \textit{viento} encabezan el bloque: \texttt{viento\_max\_semana} con \texttt{n\_eventos} ($\rho = 0{.}139$) y \texttt{viento\_medio\_semana} con \texttt{n\_eventos} ($\rho = 0{.}135$), con valores del mismo orden frente a \texttt{usuarios\_afectados} ($\rho \approx 0{.}12$), \texttt{elementos\_afectados} ($\rho \approx 0{.}11$) y \texttt{duracion\_total} ($\rho \approx 0{.}11$). La \texttt{precip\_total\_semana} le sigue con asociaciones positivas débiles ($\rho = 0{.}069$ con \texttt{duracion\_total} y $\rho = 0{.}065$ con \texttt{n\_eventos}), mientras la humedad ($\rho \approx 0{.}05$) y las temperaturas ($|\rho| < 0{.}05$, con signo negativo) resultan prácticamente irrelevantes en el grano semanal.

    Conviene precisar el alcance de este resultado. La magnitud reducida de la asociación no implica ausencia de efecto físico de la lluvia sobre la red, sino que el acumulado semanal por cuadrante es un descriptor grueso del fenómeno: promedia días secos y días de tormenta, y no captura la intensidad horaria ni la coincidencia temporal exacta entre el evento meteorológico y la desconexión. Con este descriptor, la asociación bivariada disponible en el grano cuadrante~$\times$~semana es débil, y así se reporta.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/barras_correlacion_clima_operativo_mod.png}
        \caption{Coeficientes de Spearman entre variables climáticas y operativas en $D_{\mathrm{mod}}$.}
        \label{fig:barras_cor_clima_operativo}
    \end{figure}
    
    \begin{table}[!htbp]
    \centering
    \small
    \caption{Asociación Spearman entre clima y variables operativas: pares de mayor magnitud ($D_{\mathrm{mod}}$; panel completo)}
    \label{tab:clima_operativo_cor}
    \begin{tabular}{@{}l r r@{}}
    \toprule
    \textbf{Par de variables} & \textbf{Spearman $\rho$} & \textbf{$p$-valor} \\
    \midrule
    \texttt{viento\_max\_semana} $\leftrightarrow$ \texttt{n\_eventos}            & 0.139 & $< 0{.}001$ \\
    \texttt{viento\_medio\_semana} $\leftrightarrow$ \texttt{n\_eventos}           & 0.135 & $< 0{.}001$ \\
    \texttt{viento\_max\_semana} $\leftrightarrow$ \texttt{usuarios\_afectados}    & 0.125 & $< 0{.}001$ \\
    \texttt{viento\_max\_semana} $\leftrightarrow$ \texttt{duracion\_total}        & 0.112 & $< 0{.}001$ \\
    \texttt{precip\_total\_semana} $\leftrightarrow$ \texttt{duracion\_total}      & 0.069 & $< 0{.}001$ \\
    \texttt{precip\_total\_semana} $\leftrightarrow$ \texttt{n\_eventos}           & 0.065 & $< 0{.}001$ \\
    \texttt{precip\_total\_semana} $\leftrightarrow$ \texttt{elementos\_afectados} & 0.056 & $< 0{.}001$ \\
    \texttt{precip\_total\_semana} $\leftrightarrow$ \texttt{usuarios\_afectados}  & 0.053 & $< 0{.}001$ \\
    \texttt{humedad\_media\_semana} $\leftrightarrow$ \texttt{n\_eventos}          & 0.054 & $< 0{.}001$ \\
    \texttt{temp\_media\_semana} $\leftrightarrow$ \texttt{n\_eventos}             & $-0.047$ & $< 0{.}001$ \\
    \bottomrule
    \end{tabular}
    \end{table}
    
    \paragraph{Análisis condicionado.} Dado que el $63$~\% de las filas cuadrante~$\times$~semana registra \texttt{n\_eventos} $= 0$, se repitió el análisis sobre el subconjunto con al menos un evento ($13{.}351$ filas; $37$~\% del panel). En este régimen la asociación precipitación--\texttt{n\_eventos} se atenúa aún más ($\rho = 0{.}029$) y el viento conserva la primacía ($\rho = 0{.}097$ para \texttt{viento\_max\_semana}), lo que indica que la escasa señal climática se concentra en la transición entre semanas sin y con interrupción, y no en la intensidad condicional de los eventos. La Figura~\ref{fig:heatmap_cor_condicionado} resume las correlaciones en este subconjunto.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.92\textwidth]{img/heatmap_correlacion_condicionado_mod.png}
        \caption{Correlaciones de Spearman en semanas-cuadrante con \texttt{n\_eventos} $> 0$ ($n = 13{.}351$).}
        \label{fig:heatmap_cor_condicionado}
    \end{figure}
    
    \noindent En síntesis, el análisis de correlación sobre $D_{\mathrm{mod}}$ confirma: (i)~\textit{alta redundancia} entre variables operativas de severidad y entre temperaturas/vientos del bloque climático; (ii)~una \textit{señal climática débil en términos bivariados}, encabezada por el viento ($\rho \approx 0{.}14$) y seguida por la precipitación ($\rho \approx 0{.}07$); y (iii)~\textit{asociaciones prácticamente nulas} de temperatura y humedad en el grano semanal. Estos hallazgos fundamentan conservar \texttt{viento\_medio\_semana} y \texttt{precip\_total\_semana} como candidatas climáticas, sujetas a la regla de disponibilidad \emph{ex ante} (rezago o pronóstico; Sección~\ref{sec:metodos-modelos}), la exclusión de variables climáticas redundantes y la fijación de la respuesta en \texttt{n\_eventos} y/o \texttt{duracion\_total}, dejando \texttt{usuarios\_afectados} y \texttt{elementos\_afectados} fuera del conjunto de predictoras contemporáneas (impacto posterior). Asimismo advierten que la Etapa~4 no debe apoyarse únicamente en covariables meteorológicas de la misma semana: la estructura temporal y territorial de la propia serie de eventos (Secciones~\ref{sec:analisis_temporal_eventos} y~\ref{sec:segmentacion_exploratoria}) concentra la mayor parte de la señal predictiva disponible. El análisis multivariado formal (VIF, redundancia simultánea) se desarrolla en la Sección~\ref{sec:evaluacion_multivariada}.
    
\subsection{Evaluación inicial de relaciones multivariadas}
\label{sec:evaluacion_multivariada}
    Con fundamento en las redundancias bivariadas identificadas en la Sección~\ref{sec:analisis_correlacion_asociacion}, se evaluaron las relaciones \textit{simultáneas} entre covariables sobre $D_{\mathrm{mod}}$ mediante VIF, PCA exploratorio, agrupamiento jerárquico de variables climáticas, correlación parcial y un GLM Poisson de diagnóstico (Sección~\ref{sec:metodos-modelos}). El procedimiento reproducible se documenta en el Apéndice~\ref{app:html-multivariado}.
    
    \paragraph{Multicolinealidad climática (VIF).} Se estimó un modelo auxiliar lineal \texttt{n\_eventos $\sim$} bloque climático completo para calcular el VIF de cada covariable. La Figura~\ref{fig:vif_clima_multivariado} muestra VIF elevados en \texttt{temp\_media\_semana} ($43{.}92$), \texttt{viento\_medio\_semana} ($24{.}22$), \texttt{viento\_max\_semana} ($22{.}58$), \texttt{temp\_max\_semana} ($17{.}27$) y \texttt{temp\_min\_semana} ($14{.}37$), mientras \texttt{precip\_total\_semana} ($1{.}50$) presenta inflación baja y \texttt{humedad\_media\_semana} ($5{.}68$) apenas moderada. Al restringir el bloque a \texttt{precip\_total\_semana}, \texttt{temp\_media\_semana}, \texttt{humedad\_media\_semana} y \texttt{viento\_medio\_semana}, los VIF descienden a valores aceptables (máximo $7{.}21$ en temperatura media; Tabla~\ref{tab:vif_reducido}).
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.92\textwidth]{img/vif_clima_multivariado_mod.png}
        \caption{Inflación de varianza en el bloque climático (modelo auxiliar \texttt{n\_eventos $\sim$ clima}; $D_{\mathrm{mod}}$).}
        \label{fig:vif_clima_multivariado}
    \end{figure}
    
    \begin{table}[!htbp]
    \centering
    \small
    \caption{Inflación de varianza con subconjunto climático reducido propuesto para la Etapa~4}
    \label{tab:vif_reducido}
    \begin{tabular}{@{}l r@{}}
    \toprule
    \textbf{Variable} & \textbf{VIF} \\
    \midrule
    \texttt{temp\_media\_semana}     & 7.21 \\
    \texttt{humedad\_media\_semana}  & 4.47 \\
    \texttt{viento\_medio\_semana}   & 3.27 \\
    \texttt{precip\_total\_semana}   & 1.49 \\
    \bottomrule
    \end{tabular}
    \end{table}
    
    \paragraph{PCA exploratorio del bloque climático.}
    El PCA se aplica aquí como diagnóstico de redundancia entre covariables climáticas, no como fuente de predictoras finales, conforme a lo formalizado en la Sección~\ref{sec:metodos-modelos}.
    
    \subparagraph{Variables de entrada.} Ingresan las siete variables climáticas semanales de $D_{\mathrm{mod}}$ ($n = 36{.}120$ filas cuadrante~$\times$~semana; $p = 7$):
    \texttt{temp\_media\_semana}, \texttt{temp\_max\_semana}, \texttt{temp\_min\_semana}, \texttt{humedad\_media\_semana}, \texttt{viento\_medio\_semana}, \texttt{viento\_max\_semana} y \texttt{precip\_total\_semana}.
    
    \subparagraph{Condiciones de aplicación.} Las siete son cuantitativas continuas; $n\gg p$; no hay faltantes en $D_{\mathrm{mod}}$; y se homogenizan mediante estandarización ($z$-scores; Sección~\ref{sec:metodos-modelos}) porque tienen distintas unidades (\textdegree C, \%, m/s, mm). El PCA se ejecuta sobre la matriz de correlaciones implícita en la estandarización (\texttt{prcomp} sobre la matriz centrada y escalada). La asociación previa entre variables (Sección~\ref{sec:analisis_correlacion_asociacion}) justifica el análisis: temperaturas y vientos están fuertemente intercorrelacionados, mientras la precipitación es relativamente independiente. Se reconoce que \texttt{precip\_total\_semana} es asimétrica; el ejercicio permanece exploratorio y se interpreta junto con VIF y el dendrograma.
    
    \subparagraph{Varianza retenida y pesos (loadings).} Las dos primeras componentes condensan el $87{.}3$~\% de la varianza (PC1: $66{.}9$~\%; PC2: $20{.}4$~\%) y PC3 aporta un $8{.}3$~\% adicional (acumulado $95{.}6$~\%; Figura~\ref{fig:pca_scree_clima}). La Tabla~\ref{tab:pca_loadings_clima} y la Figura~\ref{fig:pca_loadings_clima} reportan las cargas; se interpretan como variables relevantes en cada componente aquellas con $|loading|\geq 0{.}40$ (Sección~\ref{sec:metodos-modelos}):
    \begin{itemize}
        \item \textit{PC1 (eje térmico--viento):} dominan las temperaturas (\texttt{temp\_media\_semana} $0{.}45$, \texttt{temp\_max\_semana} $0{.}43$, \texttt{temp\_min\_semana} $0{.}43$), con cargas negativas de viento (\texttt{viento\_medio\_semana} $-0{.}40$, \texttt{viento\_max\_semana} $-0{.}39$). La precipitación es irrelevante en este eje ($-0{.}08$).
        \item \textit{PC2 (eje de humedad--lluvia):} la variable más importante es \texttt{precip\_total\_semana} ($0{.}70$), seguida de \texttt{humedad\_media\_semana} ($0{.}51$).
        \item \textit{PC3:} contrastan precipitación ($-0{.}66$) y viento máximo ($-0{.}43$) frente a humedad ($0{.}32$), con contribución adicional del viento medio ($-0{.}37$).
    \end{itemize}
    En síntesis, las temperaturas (y, en sentido opuesto, el viento) concentran el peso en PC1, mientras la precipitación, variable de mayor interés operativo, concentra su importancia en PC2--PC3. Ello respalda conservar precipitación como predictora no redundante y depurar temperaturas/vientos extremos del conjunto reducido.
    
    \begin{table}[!htbp]
    \centering
    \small
    \caption{Cargas (loadings) del PCA climático: peso de cada variable por componente ($D_{\mathrm{mod}}$; PC1--PC3)}
    \label{tab:pca_loadings_clima}
    \begin{tabular}{@{}l r r r@{}}
    \toprule
    \textbf{Variable} & \textbf{PC1} & \textbf{PC2} & \textbf{PC3} \\
    \midrule
    \texttt{temp\_media\_semana}    &  0.45 & $-0.07$ & $-0.22$ \\
    \texttt{temp\_max\_semana}      &  0.43 & $-0.12$ & $-0.28$ \\
    \texttt{temp\_min\_semana}      &  0.43 &  0.11  & $-0.10$ \\
    \texttt{humedad\_media\_semana} & $-0.33$ &  0.51  &  0.32  \\
    \texttt{viento\_medio\_semana}  & $-0.40$ & $-0.33$ & $-0.37$ \\
    \texttt{viento\_max\_semana}    & $-0.39$ & $-0.33$ & $-0.43$ \\
    \texttt{precip\_total\_semana}  & $-0.08$ &  0.70  & $-0.66$ \\
    \bottomrule
    \end{tabular}
    
    \noindent\textit{Nota:} se destacan como relevantes en la interpretación las cargas con valor absoluto $\geq 0{.}40$.
    \end{table}

    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.82\textwidth]{img/pca_scree_clima_multivariado_mod.png}
        \caption{Varianza acumulada del bloque climático por componente principal.}
        \label{fig:pca_scree_clima}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/pca_loadings_clima_multivariado_mod.png}
        \caption{Peso (loadings) de las variables climáticas en las tres primeras componentes principales.}
        \label{fig:pca_loadings_clima}
    \end{figure}
    
    \paragraph{Agrupamiento jerárquico.} El dendrograma de la Figura~\ref{fig:dendrograma_clima} confirma dos clusters principales: (i)~temperaturas y vientos, altamente intercorrelados; y (ii)~\texttt{precip\_total\_semana}, relativamente aislada. Este resultado valida la exclusión de \texttt{temp\_max\_semana}, \texttt{temp\_min\_semana} y \texttt{viento\_max\_semana} del conjunto predictor reducido.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/dendrograma_clima_multivariado_mod.png}
        \caption{Agrupamiento de redundancias entre variables climáticas (Sección~\ref{sec:metodos-modelos}).}
        \label{fig:dendrograma_clima}
    \end{figure}
    
    \paragraph{Asociación multivariada clima $\rightarrow$ \texttt{n\_eventos}.} La correlación parcial de Spearman entre precipitación y \texttt{n\_eventos}, controlando \texttt{temp\_media\_semana} y \texttt{viento\_medio\_semana}, es $\rho = 0{.}114$, casi el doble del valor bivariado ($\rho = 0{.}065$). El aumento al condicionar indica que temperatura y viento actuaban como confusores que enmascaraban parcialmente la señal de lluvia; aun así, la magnitud sigue siendo modesta. Un GLM Poisson exploratorio con el subconjunto reducido (Figura~\ref{fig:glm_coef_multivariado}) arroja coeficientes positivos y significativos para las cuatro covariables, con la temperatura media estandarizada ($\hat{\beta} = 0{.}81$) y el viento medio estandarizado ($\hat{\beta} = 0{.}73$) como efectos dominantes, seguidos de la humedad ($\hat{\beta} = 0{.}35$) y de la precipitación en transformación logarítmica desplazada ($\hat{\beta} = 0{.}13$; todos con $p < 0{.}001$). Este contraste con el orden bivariado, donde las temperaturas eran irrelevantes, es consistente con la multicolinealidad diagnosticada por el VIF y refuerza que los coeficientes del bloque climático completo no admiten lectura causal individual; se reportan como diagnóstico exploratorio.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.82\textwidth]{img/glm_coef_clima_multivariado_mod.png}
        \caption{Efecto conjunto del clima reducido sobre la frecuencia de eventos (diagnóstico EDA).}
        \label{fig:glm_coef_multivariado}
    \end{figure}
    
    \paragraph{Dimensiones del bloque operativo.}
    De forma análoga, se aplica un PCA exploratorio al bloque operativo de $D_{\mathrm{mod}}$ para delimitar dimensiones de impacto, no para construir predictoras.
    
    \subparagraph{Variables de entrada y condiciones.} Ingresan las cuatro métricas \texttt{n\_eventos}, \texttt{usuarios\_afectados}, \texttt{elementos\_afectados} y \texttt{duracion\_total} ($n = 36{.}120$; $p = 4$), tras transformación logarítmica desplazada y estandarización (Sección~\ref{sec:metodos-modelos}), dadas la cero-inflación, la asimetría y las distintas escalas. Son cuantitativas, $n\gg p$ y no presentan faltantes.
    
    \subparagraph{Pesos e interpretación.} PC1 concentra el $83{.}8$~\% de la varianza, con cargas elevadas y del mismo signo en las cuatro variables (Tabla~\ref{tab:pca_loadings_operativo}; Figura~\ref{fig:pca_loadings_operativo}): las más importantes son \texttt{usuarios\_afectados} ($0{.}53$) y \texttt{n\_eventos} ($0{.}52$), seguidas de \texttt{elementos\_afectados} ($0{.}50$) y \texttt{duracion\_total} ($0{.}45$). PC2 ($11{.}9$~\%; acumulado $95{.}7$~\%) contrasta duración ($0{.}81$) frente a afectación (\texttt{elementos\_afectados} $-0{.}48$; \texttt{usuarios\_afectados} $-0{.}33$). Ello respalda no emplear simultáneamente las cuatro variables operativas como respuestas ni como predictoras, y construir $SAIFI$ y $SAIDI$ en la Etapa~4 a partir de \texttt{usuarios\_afectados} y \texttt{duracion\_total}. \texttt{usuarios\_afectados} entra en el numerador de $Y$, pero queda excluido como predictora contemporánea para evitar fuga de información; \texttt{elementos\_afectados} se reserva al diagnóstico de severidad (Sección~\ref{sec:metodos-modelos}).
    
    \begin{table}[!htbp]
    \centering
    \small
    \caption{Cargas (loadings) del PCA operativo en escala logarítmica desplazada ($D_{\mathrm{mod}}$; PC1--PC2)}
    \label{tab:pca_loadings_operativo}
    \begin{tabular}{@{}l r r@{}}
    \toprule
    \textbf{Variable} & \textbf{PC1} & \textbf{PC2} \\
    \midrule
    \texttt{n\_eventos}            & 0.52 &  0.09 \\
    \texttt{usuarios\_afectados}  & 0.53 & $-0.33$ \\
    \texttt{elementos\_afectados} & 0.50 & $-0.48$ \\
    \texttt{duracion\_total}      & 0.45 &  0.81 \\
    \bottomrule
    \end{tabular}
    
    \noindent\textit{Nota:} se interpretan como relevantes las cargas con valor absoluto $\geq 0{.}40$.
    \end{table}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.82\textwidth]{img/pca_loadings_operativo_multivariado_mod.png}
        \caption{Peso (loadings) de frecuencia, afectación y duración en las dos primeras componentes del bloque operativo.}
        \label{fig:pca_loadings_operativo}
    \end{figure}
    
    \noindent En síntesis, la evaluación multivariada sobre $D_{\mathrm{mod}}$ recomienda: (i)~predictoras climáticas \texttt{precip\_total\_semana}, \texttt{temp\_media\_semana}, \texttt{humedad\_media\_semana} y \texttt{viento\_medio\_semana}, justificadas por loadings, VIF y dendrograma (el PCA no se usa como modelo predictivo, sino como diagnóstico de redundancia); (ii)~$SAIFI$ y $SAIDI$ como variables respuesta del modelado, construidas en la Etapa~4; (iii)~exclusión de temperaturas extremas y viento máximo por redundancia simultánea; (iv)~expectativas moderadas sobre la capacidad explicativa del bloque meteorológico contemporáneo, dado el tamaño reducido de las asociaciones observadas, lo que hace aconsejable complementarlo con rezagos de la propia serie de eventos y con la estratificación territorial; y (v)~uso de \texttt{usuarios\_afectados} solo en el numerador de $Y$, no como predictora contemporánea. Estos criterios se operacionalizan en la depuración cuantitativa y la construcción de matrices de diseño de la Etapa~4.
    
\subsection{Segmentación exploratoria}
\label{sec:segmentacion_exploratoria}
    Con fundamento en la segmentación territorial introducida en la Sección~\ref{sec:identificacion_patrones_tendencias}, se profundizó la heterogeneidad de $D_{\mathrm{mod}}$ mediante perfiles comparativos por nivel de actividad, evolución 2024--2025, estratificación por régimen de precipitación semanal y clustering multivariado de cuadrantes. El procedimiento reproducible se documenta en el Apéndice~\ref{app:html-segmentacion}.
    
    \paragraph{Perfiles por nivel de actividad.} Sobre los $344$ cuadrantes del periodo, la clasificación Baja/Media/Alta (cuantiles P50 y P85 entre cuadrantes con eventos) arroja: $162$ cuadrantes baja ($47{.}1$~\%), $113$ media ($32{.}8$~\%) y $49$ alta ($14{.}2$~\%); $20$ cuadrantes ($5{.}8$~\%) no registraron eventos. Los perfiles acumulados (Tabla implícita en Figura~\ref{fig:heatmap_perfiles_segmentacion}) muestran escalamiento marcado en las variables operativas: media de $22$ eventos por cuadrante en nivel baja frente a $114{.}6$ en media y $344{.}7$ en alta, con usuarios afectados medios de $1{.}588$ a $88{.}534$. El clima, en cambio, no acompaña ese escalamiento: la precipitación semanal media desciende ligeramente de $19{.}6$~mm (baja) a $18{.}5$~mm (media) y $17{.}0$~mm (alta), de modo que los estratos de mayor actividad no son los más lluviosos. La Figura~\ref{fig:boxplot_nivel_segmentacion} confirma que la dispersión semanal de \texttt{n\_eventos} crece con el estrato territorial.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/boxplot_eventos_por_nivel_segmentacion_mod.png}
        \caption{Distribución semanal de \texttt{n\_eventos} por nivel de actividad del cuadrante ($D_{\mathrm{mod}}$).}
        \label{fig:boxplot_nivel_segmentacion}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.88\textwidth]{img/heatmap_perfiles_nivel_segmentacion_mod.png}
        \caption{Perfil normalizado por nivel de actividad (eventos, usuarios, duración, semanas activas, clima).}
        \label{fig:heatmap_perfiles_segmentacion}
    \end{figure}
    
    \paragraph{Evolución 2024 vs 2025 por estrato.} El total territorial ascendió de $15{.}211$ eventos en 2024 ($287$/semana) a $18{.}199$ en 2025 ($350$/semana). Al desagregar por nivel fijado en el periodo completo, el ratio medio 2025/2024 es homogéneo: $1{.}24$ (baja), $1{.}26$ (media) y $1{.}24$ (alta), lo que indica incremento transversal sin reordenamiento fuerte de la jerarquía territorial (Figura~\ref{fig:barras_anio_nivel_segmentacion}). El crecimiento observado en 2024--2025 debe interpretarse en coherencia con la homogeneización de codificación activo/sitio bajo OMS-SP7 (Sección~\ref{sec:delimitacion_temporal_modelado}), no exclusivamente como deterioro operativo.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.85\textwidth]{img/barras_eventos_anio_nivel_segmentacion_mod.png}
        \caption{Eventos acumulados por nivel de actividad y año en $D_{\mathrm{mod}}$.}
        \label{fig:barras_anio_nivel_segmentacion}
    \end{figure}
    
    \paragraph{Régimen climático semanal.} Se estratificaron las filas según el percentil~$75$ de \texttt{precip\_total\_semana} ($24{.}7$~mm). Las semanas lluviosas ($25$~\% de las filas; $9{.}030$ observaciones) presentan media de \texttt{n\_eventos} de $0{.}918$, prácticamente igual a la de las semanas secas ($0{.}927$; $75$~\%; $27{.}090$ filas), con $37{.}7$~\% frente a $36{.}7$~\% de semanas con al menos un evento. La prueba de Wilcoxon no detecta diferencia significativa ($p = 0{.}175$; Figura~\ref{fig:boxplot_regimen_segmentacion}), resultado consistente con la débil asociación bivariada precipitación--eventos de la Sección~\ref{sec:analisis_correlacion_asociacion}. La dicotomización por umbral resulta así insuficiente para separar regímenes operativos: si la lluvia tiene efecto, opera a través de la intensidad de episodios puntuales y no del acumulado semanal, lo que sugiere explorar en la Etapa~4 indicadores de intensidad diaria o de número de días de lluvia dentro de la semana.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.82\textwidth]{img/boxplot_eventos_regimen_lluvia_segmentacion_mod.png}
        \caption{\texttt{n\_eventos} según régimen de precipitación semanal (umbral P75).}
        \label{fig:boxplot_regimen_segmentacion}
    \end{figure}
    
    \paragraph{Clustering multivariado de cuadrantes.} Se aplicó k-means ($k = 4$) sobre cuadrantes con al menos un evento, con variables estandarizadas de frecuencia, severidad, duración, clima y altitud (Sección~\ref{sec:metodos-modelos}; excluyendo covariables de varianza nula). El mapa de la Figura~\ref{fig:mapa_cluster_segmentacion} y el heatmap de la Figura~\ref{fig:heatmap_cluster_segmentacion} distinguen: C3 ($1$ cuadrante; $2{.}224$ eventos; hub extremo), C1 ($139$ cuadrantes; media $107{.}4$ eventos; mayor severidad y duración acumulada, altitud media $496$~m), C4 ($126$ cuadrantes; media $97{.}7$ eventos; el estrato de mayor altitud, $1{.}741$~m) y C2 ($58$ cuadrantes; media $68{.}0$ eventos; el más lluvioso, $26{.}2$~mm semanales, y el de menor duración acumulada). Que el cluster más lluvioso sea también el de menor actividad refuerza que la precipitación no ordena por sí sola el territorio; la altitud y el perfil de severidad discriminan más. La existencia de un hub aislado y de estratos con perfiles distinguibles respalda evaluar modelado global frente a estratificación por cluster en la Etapa~4.
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.95\textwidth]{img/mapa_clusters_cuadrantes_segmentacion_mod.png}
        \caption{Perfiles territoriales de cuadrantes según actividad y clima ($k = 4$; $D_{\mathrm{mod}}$).}
        \label{fig:mapa_cluster_segmentacion}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.88\textwidth]{img/heatmap_perfiles_cluster_segmentacion_mod.png}
        \caption{Perfil medio por cluster de cuadrantes (variables operativas y climáticas).}
        \label{fig:heatmap_cluster_segmentacion}
    \end{figure}
    
    \noindent En síntesis, la segmentación exploratoria sobre $D_{\mathrm{mod}}$ evidencia: (i)~heterogeneidad territorial persistente (14{.}2~\% de cuadrantes en nivel alta concentran actividad desproporcionada); (ii)~incremento homogéneo 2024$\rightarrow$2025 en todos los estratos; (iii)~\textit{ausencia} de diferencia significativa en la frecuencia de interrupciones entre semanas lluviosas y secas cuando la lluvia se mide como acumulado semanal; y (iv)~cuatro perfiles espaciales distinguibles por clustering, ordenados principalmente por severidad y altitud más que por régimen de lluvia. Estos criterios orientan la estratificación territorial y la partición entrenamiento/prueba de la Etapa~4.
    
\subsection{Resultados y entregables de la etapa}
\label{sec:resultados_eda}
    La Etapa~3 sobre $D_{\mathrm{mod}}$ (2024--2025; $36{.}120$ filas cuadrante~$\times$~semana) produjo un diagnóstico exploratorio integrado: calidad y distribución de variables (Secciones~\ref{sec:identificacion_datos_faltantes}--\ref{sec:distribucion_variables}), delimitación temporal y materialización del panel de modelado (Sección~\ref{sec:analisis_temporal_eventos}), patrones y visualización (Sección~\ref{sec:identificacion_patrones_tendencias}), correlación bivariada (Sección~\ref{sec:analisis_correlacion_asociacion}), evaluación multivariada con recomendación de predictoras y respuesta (Sección~\ref{sec:evaluacion_multivariada}) y segmentación territorial (Sección~\ref{sec:segmentacion_exploratoria}). Los hallazgos convergentes son: (i)~señal climática contemporánea \textit{débil} en el grano semanal (viento por delante de precipitación); (ii)~\textit{redundancia} en bloques operativo y climático que justifica subconjuntos reducidos, con la severidad (\texttt{usuarios\_afectados}, \texttt{elementos\_afectados}) altamente alineada a \texttt{n\_eventos} en parte por construcción; (iii)~\textit{taxonomía de tres grupos} (Sección~\ref{sec:metodos-modelos}): predictoras disponibles \emph{ex ante}, respuestas $SAIFI$/$SAIDI$, e impacto posterior excluido como predictor contemporáneo; (iv)~\textit{heterogeneidad espacial} y clusters territoriales que sugieren estratificación en el modelado; y (v)~\textit{partición natural} entrenamiento 2024 / prueba 2025 bajo cobertura homogénea de emparejamiento evento--elemento. Los informes HTML reproducibles se consolidan en el Apéndice~\ref{app:informes-html}; el \textit{entregable ejecutivo consolidado} corresponde al Apéndice~\ref{app:html-eda} (síntesis de las Secciones~\ref{sec:identificacion_datos_faltantes}--\ref{sec:resultados_eda} con hallazgos, figuras representativas y lineamientos para la Etapa~4). La ingeniería de variables de la Etapa~4 operacionaliza esa taxonomía, incluidos rezagos climáticos/operativos y, si procede, la reelaboración de la métrica de severidad solo con fines descriptivos,.
    
    \textit{Resultado:} conocimiento exploratorio del comportamiento de los datos y hallazgos relevantes para el modelado.
    
    \textit{Entregable:} Documento de análisis descriptivo y exploratorio (EDA), Apéndice~\ref{app:html-eda}.
    

\section{Etapa 4 – Ingeniería de variables y preparación del modelo}
\label{sec:metodología-etapa4}

A partir de los hallazgos del EDA y de la taxonomía de la Sección~\ref{sec:metodos-modelos}, en esta etapa se transforma el dataset consolidado en una representación analítica adecuada para el entrenamiento de modelos. El criterio rector es evitar \textit{fuga de información}: solo entran como predictoras variables disponibles \emph{antes} de la semana~$t$ a pronosticar. Las actividades incluyen la definición formal de la variable respuesta, la construcción de predictoras informadas por el EDA (clima rezagado o pronosticado, histórico de eventos, rasgos del cuadrante, estacionalidad), la depuración y codificación de covariables, la estandarización de variables continuas y la partición temporal entrenamiento/prueba. \texttt{usuarios\_afectados} y \texttt{elementos\_afectados} se tratan como \textit{impacto posterior}: el primero entra en el numerador de $SAIFI$ y $SAIDI$, pero ninguno se habilita como predictora contemporánea. El preprocesamiento, incluida la selección de variables con base en el EDA, se concentra aquí, en coherencia con estabilidad numérica, identificabilidad y validez operativa del pronóstico.

Las actividades desarrolladas en esta etapa se describen en las subsecciones siguientes.

\subsection{Construcción de variables predictoras}
Se construyen únicamente variables del grupo \textit{disponibles antes del periodo a pronosticar}: (i)~clima rezagado ($t-1,\ldots,t-h$) o, cuando exista, clima pronosticado para~$t$ con fecha de emisión anterior al inicio de la semana; (ii)~histórico de eventos del cuadrante (conteos o duraciones rezagadas) y, si procede, de vecinos espaciales; (iii)~características del cuadrante enriquecidas en la Sección~\ref{sec:enriquecimiento_panel_cuadrante} (infraestructura, estrato municipal, topología de red, edad/modificaciones de activos); (iv)~exposición externa (NDVI, UNGRD, sismicidad); (v)~estacionalidad y calendario (mes, semana del año, indicadores de régimen). Quedan \textit{excluidas} como predictoras contemporáneas \texttt{usuarios\_afectados}$_t$, \texttt{elementos\_afectados}$_t$, \texttt{n\_eventos}$_t$ y \texttt{duracion\_total}$_t$ al pronosticar $\mathrm{SAIFI}_t$ o $\mathrm{SAIDI}_t$.

\subsection{Construcción de la variable respuesta}
Las respuestas son $Y_t=\mathrm{SAIFI}_t$ y $Y'_t=\mathrm{SAIDI}_t$, calculadas en cada fila cuadrante~$\times$~semana como
\begin{equation}
\mathrm{SAIFI}_{q,s}=\dfrac{\texttt{usuarios\_afectados}_{q,s}}{UT_{m(s)}},
\qquad
\mathrm{SAIDI}_{q,s}=\dfrac{\texttt{duracion\_total}_{q,s}\times\texttt{usuarios\_afectados}_{q,s}}{UT_{m(s)}},
\end{equation}
donde $UT_{m(s)}$ es el valor mensual de usuarios totales del sistema (\texttt{CS1\_UT\_M} en \texttt{data\_usuarios\_totales\_sistema.csv}) correspondiente al mes calendario de la fecha de inicio de la semana~$s$. \texttt{n\_eventos} y \texttt{duracion\_total} no se modelan como $Y$: permanecen como insumos de construcción y como historial rezagado. \texttt{elementos\_afectados} no entra en $Y$ ni como predictora contemporánea.

\subsection{Análisis de asociación y depuración de variables cuantitativas}
\label{sec:asociacion_depuracion_cuanti}
Se define el conjunto candidato \emph{ex ante} sobre $D_{\mathrm{mod}}$ (clima rezagado, histórico de eventos, rasgos del cuadrante, exposición y estacionalidad) y se excluyen, por diseño, el clima contemporáneo, la respuesta alterna contemporánea y el impacto posterior (\texttt{usuarios\_afectados}, \texttt{elementos\_afectados}). La depuración se estima solo con el entrenamiento (2024) y combina filtros de cobertura, varianza casi nula, asociación de Spearman con corrección FDR, agrupamiento por redundancia ($|\rho|\geq 0{.}70$) y eliminación iterativa por VIF (umbral~5). No se borran variables del panel: se depuran de la matriz de diseño.

De~$27$ candidatas se retienen~$13$ (Figuras~\ref{fig:asoc_exante_cuanti}--\ref{fig:vif_cuanti_depurado}): precipitación rezagada y acumulada, humedad rezagada, viento máximo rezagado, rezagos de \texttt{n\_eventos}, estacionalidad armónica, altitud, longitud y $\log(\texttt{n\_activos}+1)$ como \emph{offset}. Las temperaturas y los promedios móviles del histórico se descartan por redundancia o multicolinealidad. Detalle reproducible en el Apéndice~\ref{app:html-asociacion-cuanti}.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{img/asociacion_candidatas_exante_mod.png}
    \caption{Asociación de Spearman entre candidatas \emph{ex ante} y $SAIFI$ (entrenamiento 2024).}
    \label{fig:asoc_exante_cuanti}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{img/dendrograma_candidatas_exante_mod.png}
    \caption{Agrupamiento jerárquico de candidatas ($1-|\rho|$ Spearman); la línea punteada marca el umbral $|\rho|=0{.}70$.}
    \label{fig:dendro_exante_cuanti}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{img/vif_cuanti_depurado_mod.png}
    \caption{VIF del conjunto cuantitativo retenido tras la depuración iterativa (umbral~5).}
    \label{fig:vif_cuanti_depurado}
\end{figure}

\subsection{Análisis de asociación y depuración de variables cualitativas}
Las cualitativas candidatas (\texttt{estrato\_predominante\_municipal}, tipología de zona, indicadores de régimen lluvioso/seco y flags de exposición) se depuran por cobertura de categorías y por asociación con $SAIFI$/$SAIDI$ en el entrenamiento. Categorías raras se colapsan; las variables sin contraste útil se excluyen de la matriz de diseño sin borrarlas del panel. El procedimiento reproducible se documenta en el Apéndice~\ref{app:html-asociacion-cuanti}.

\subsection{Codificación de variables categóricas}
Las categóricas retenidas se codifican como \emph{dummy} de referencia (contrastes tratamiento) o, cuando el orden es informativo (estrato), como factor ordinal. No se usan codificaciones posteriores al corte de prueba. En el panel de modelado espacial, las proporciones de estrato (\texttt{prop\_estrato\_*}, \texttt{prop\_no\_residencial}) entran ya como proporciones numéricas y no requieren \emph{dummy} adicional.

\subsection{Estandarización de variables cuantitativas}
Las cuantitativas continuas se estandarizan (\emph{z}-score) solo cuando el algoritmo lo exige (p.~ej.~regularización o distancias); en OLS/FE de \texttt{fixest} se mantienen en escala original para interpretar coeficientes. Las respuestas $SAIDI$ y $SAIFI$ se transforman con $\log(1+y)$ ante asimetría y ceros, según \texttt{Etapa5\_01}/\texttt{Etapa5\_02}.

\subsection{Generación de ventanas temporales}
Por cuadrante se construyen rezagos $t-1,\ldots,t-h$ de clima y de la respuesta, medias móviles (p.~ej.~precipitación y viento a 4 semanas) e indicadores de calendario (mes, trimestre), a partir de la persistencia temporal identificada en el análisis exploratorio (Sección~\ref{sec:analisis_temporal_eventos}), que motiva incluir al menos el primer rezago de la respuesta como predictora \emph{ex ante}.

\subsection{Partición de datos en entrenamiento y prueba}
Con fundamento en el diagnóstico de cobertura del emparejamiento evento--elemento (Sección~\ref{sec:delimitacion_temporal_modelado}), el universo de estimación se restringe a $D_{\mathrm{mod}}$: observaciones cuadrante$\times$~semana con \texttt{year(semana)} $\in \{2024,\,2025\}$, periodo en el que la coincidencia entre \texttt{COMPONENTE\_FALLADO} y el inventario GIS (\texttt{tipo\_elemento.csv}) alcanza al menos el $79{.}8$~\%, frente a $\approx 31$~\% en 2021--2023 bajo OMS-SPARD. La partición temporal propuesta asigna las observaciones de \textit{2024} al conjunto de entrenamiento y las de \textit{2025} al conjunto de prueba, preservando el orden cronológico y la validez de la validación en series autocorrelacionadas (ACF rezago~$1 = 0{.}92$ en $D_{\mathrm{EDA}}$). Los años 2021--2023 no se incluyen en el entrenamiento ni en la prueba del modelo predictivo, dado el sesgo por incompatibilidad entre código de activo y código de sitio. En los modelos espaciales de \texttt{Etapa5\_01}/\texttt{Etapa5\_02} se aplica además un corte temporal del 15~\% final de semanas como \emph{holdout}, nunca una partición aleatoria.

\subsection{Construcción de matrices de diseño}
La matriz de diseño retiene las predictoras \emph{ex ante} depuradas: clima (contemporáneo o rezagado según la especificación), NDVI, desastres UNGRD, sismicidad, topología de red, composición socioeconómica, edad/modificaciones de activos, rezagos de la respuesta y rezagos espaciales de vecinos. Se excluyen impacto posterior y respuestas contemporáneas cruzadas. Los insumos se materializan en \texttt{data\_final\_modelo\_cuadrantes\_reales.rds} (Apéndices~\ref{app:html-etapa5-01}--\ref{app:html-etapa5-02}).

\subsection{Reducción de dimensionalidad}
Además del filtrado por Spearman/VIF (Sección~\ref{sec:asociacion_depuracion_cuanti}), se eliminan variables estáticas absorbidas por efectos fijos de cuadrante cuando la especificación es FE, y se retienen en Pooled OLS y en el corte transversal SAR/SEM. No se aplica PCA como paso obligatorio: la reducción se guía por redundancia y por el aporte marginal del bloque espacial.

\subsection{Segmentación analítica}
El modelado principal opera sobre semanas con falla (\texttt{hubo\_falla}$=1$) para concentrar la señal de duración/frecuencia. En paralelo, el corte transversal ponderado (suma de dígitos hacia lo reciente) agrega el panel a un vector por cuadrante para estimar SAR/SEM. La segmentación territorial del EDA (clusters) informa la interpretación, no sustituye la partición temporal.

\subsection{Transformaciones adicionales para modelado}
Se construyen: (i)~$\log(1+\mathrm{SAIDI})$ y $\log(1+\mathrm{SAIFI})$; (ii)~rezagos espaciales \texttt{SAIDI\_vecinos\_lag1}/\texttt{SAIFI\_vecinos\_lag1}, \texttt{n\_eventos\_vecinos} y \texttt{ndvi\_vecinos} vía contigüidad \emph{queen} (\texttt{spdep}); (iii)~interacciones exposición estática~$\times$~dinámica (p.~ej.~\texttt{ndvi\_x\_elementos}). Detalle en \texttt{Etapa5\_01} y \texttt{Etapa5\_02}.

\subsection{Resultados y entregables de la etapa}
\textit{Resultado:} panel analítico con predictoras \emph{ex ante}, rezagos temporales/espaciales y partición temporal documentada.

\textit{Entregable:} matriz depurada de predictoras (Sección~\ref{sec:asociacion_depuracion_cuanti}) y panel analítico final documentado en los Apéndices~\ref{app:html-etapa5-01}--\ref{app:html-etapa5-02}.

\section{Etapa 5 – Modelado predictivo y evaluativo}
\label{sec:metodología-etapa5}

En esta etapa se estiman y comparan modelos para \textit{SAIDI} y \textit{SAIFI} por separado a grano cuadrante~$\times$~semana, siguiendo \texttt{Etapa5\_01\_modelo\_SAIDI\_espacial.Rmd} y \texttt{Etapa5\_02\_modelo\_SAIFI\_espacial.Rmd} (Apéndices~\ref{app:html-etapa5-01}--\ref{app:html-etapa5-02}). El diseño combina (i)~modelos de panel para pronóstico semanal y (ii)~modelos espaciales formales SAR/SEM en corte transversal ponderado. La validación es temporal; las métricas de referencia son $RMSE$, $MAE$ y $R^2$ en escala $\log(1+y)$.

\subsection{Selección de algoritmos}
Se seleccionan cuatro familias para el panel semanal: (1)~\textit{Pooled OLS}; (2)~\textit{efectos fijos de cuadrante} (\texttt{feols}, \texttt{fixest}); (3)~\textit{efectos fijos de cuadrante y semana}; (4)~\textit{Random Forest} ($500$ árboles). En el corte transversal se estiman \textit{SAR} y \textit{SEM} (\texttt{spatialreg}). La dependencia espacial se diagnostica previamente con el índice de Moran; la Figura~\ref{fig:saidi_moran} ilustra el patrón territorial de $SAIDI$ promedio por cuadrante.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{img/saidi_moran_mapa.png}
    \caption{Mapa de $SAIDI$ promedio por cuadrante usado en el diagnóstico de autocorrelación espacial (Moran).}
    \label{fig:saidi_moran}
\end{figure}

\subsection{Entrenamiento del modelo}
El entrenamiento usa solo semanas anteriores al corte temporal. La fórmula base incluye clima semanal, precipitación móvil, NDVI, eventos catastróficos, sismicidad, rezago propio (\texttt{SAIDI\_lag1}/\texttt{SAIFI\_lag1}), topología de red, proporciones de estrato, edad/modificaciones de activos y rezagos espaciales de vecinos. SAIDI/SAIFI se modelan en escala $\log(1+y)$. La Figura~\ref{fig:saidi_split} muestra la partición temporal train/test empleada en el panel de SAIDI (análoga para SAIFI).

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{img/saidi_split_temporal.png}
    \caption{Partición temporal entrenamiento/prueba para el modelado de $SAIDI$ (holdout $\approx 15$\% final de semanas).}
    \label{fig:saidi_split}
\end{figure}

\subsection{Validación del modelo}
La validación es \textit{holdout temporal}: se reserva aproximadamente el $15$~\% final de semanas ordenadas; no se permite muestreo aleatorio. El FE de cuadrante+semana puede no ser evaluable fuera de muestra cuando las semanas de prueba no están en el entrenamiento; en ese caso se priorizan Pooled OLS, FE de cuadrante y Random Forest.

\subsection{Ajuste de hiperparámetros}
En Random Forest se fija \texttt{ntree}$=500$ con semilla reproducible. En SAR/SEM se compara AIC entre especificaciones y se verifica robustez ante colinealidad estructural. El detalle de sensibilidad queda en los HTML de \texttt{Etapa5\_01}/\texttt{Etapa5\_02}.

\subsection{Evaluación de desempeño}
Sobre el test se calculan $RMSE$, $MAE$ y $R^2$ en escala logarítmica. La Figura~\ref{fig:saidi_pred_obs} contrasta valores observados y predichos del mejor modelo de $SAIDI$; la Figura~\ref{fig:saifi_pred_obs} muestra el equivalente para $SAIFI$.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{img/saidi_pred_vs_obs.png}
    \caption{Valores observados frente a predichos de $SAIDI$ en el conjunto de prueba (escala $\log(1+y)$).}
    \label{fig:saidi_pred_obs}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{img/saifi_pred_vs_obs.png}
    \caption{Valores observados frente a predichos de $SAIFI$ en el conjunto de prueba (escala $\log(1+y)$).}
    \label{fig:saifi_pred_obs}
\end{figure}

\subsection{Comparación de modelos}
La comparación distingue dos usos: (i)~pronóstico semanal, con menor $RMSE$ en test entre Pooled OLS, FE cuadrante y Random Forest; y (ii)~explicación estructural, con menor AIC entre SAR/SEM. SAIDI y SAIFI se reportan por separado. Las tablas numéricas completas se concentran en el Capítulo~\ref{sec:resultados} y en los anexos HTML.

\subsection{Interpretación de resultados}
La interpretación combina coeficientes de las especificaciones lineales/FE, importancia de variables del Random Forest y parámetros espaciales de SAR/SEM. Las Figuras~\ref{fig:saidi_rf_imp} y~\ref{fig:saifi_rf_imp} resumen la importancia relativa de predictores en los Random Forest de $SAIDI$ y $SAIFI$.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{img/saidi_rf_importancia.png}
    \caption{Importancia de variables (\%IncMSE) del Random Forest para $SAIDI$.}
    \label{fig:saidi_rf_imp}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{img/saifi_rf_importancia.png}
    \caption{Importancia de variables (\%IncMSE) del Random Forest para $SAIFI$.}
    \label{fig:saifi_rf_imp}
\end{figure}

\subsection{Resultados y entregables de la etapa}
\textit{Resultado:} modelos de SAIDI y SAIFI entrenados, validados en holdout temporal y contrastados con especificaciones espaciales formales.

\textit{Entregables:} \texttt{Etapa5\_01\_modelo\_SAIDI\_espacial.html} y \texttt{Etapa5\_02\_modelo\_SAIFI\_espacial.html} (Apéndices~\ref{app:html-etapa5-01}--\ref{app:html-etapa5-02}).

\section{Etapa 6 – Cierre metodológico, documentación y reportes}
\label{sec:metodología-etapa6}

En la etapa final se consolidan los resultados del proyecto y se documentan de forma integral las técnicas aplicadas, las transformaciones realizadas y el código reproducible (en R o Python). El capítulo incluye apéndices técnicos, tablas de validación, bibliografía y lineamientos de reproducibilidad, así como una discusión orientada a interpretar los hallazgos en términos técnicos y operativos para la toma de decisiones del operador de red.

Las actividades desarrolladas en esta etapa se describen en las subsecciones siguientes.

\subsection{Presentación de resultados del modelo}

\subsection{Métricas de desempeño}

\subsection{Hallazgos principales}

\subsection{Interpretación técnica y operativa}

\subsection{Generación de reportes}

\subsection{Discusión de resultados}

\subsection{Limitaciones y trabajos futuros}

\subsection{Resultados y entregables de la etapa}

\textit{Resultado:} documentación final del proyecto y resultados consolidados.

\textit{Entregables:} Compendio de documentación del proyecto (informe final, informe descriptivo, informe de evaluación de modelos y reporte técnico de variables relevantes), código reproducible y documentación detallada de procesos y transformaciones.
   

\section{Relación entre etapas metodológicas y objetivos específicos}
\label{sec:etapas_objetivos}

La Tabla~\ref{tab:etapas_objetivos} sintetiza cómo cada etapa de la metodología contribuye a los objetivos específicos formulados en el Capítulo~\ref{sec:objetivos}. Los objetivos no se reformulan aquí: se indica únicamente su correspondencia con las etapas, de modo que la trazabilidad entre el diseño metodológico y el alcance del proyecto quede explícita. De forma complementaria, los informes reproducibles derivados de los archivos \texttt{EtapaX\_Y\_*.Rmd} se documentan como informes HTML en el Apéndice~\ref{app:informes-html} (subsecciones de enriquecimiento, ingeniería de variables y modelado espacial).

\begin{table}[!htbp]
\centering
\small
\caption{Correspondencia entre etapas de la metodología y objetivos específicos}
\label{tab:etapas_objetivos}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.6cm} >{\centering\arraybackslash}p{1.6cm} >{\raggedright\arraybackslash}p{7.6cm}@{}}
\toprule
\textbf{Etapa} & \textbf{Objetivo(s)} & \textbf{Aporte metodológico} \\
\midrule
Etapa~1 -- Extracción de datos
  (Sección~\ref{sec:metodología-etapa1})
  & OE1
  & Recolección e integración de fuentes internas y externas; repositorio analítico unificado y catálogo de calidad. \\
\addlinespace
Etapa~2 -- Integración y depuración
  (Sección~\ref{sec:metodología-etapa2})
  & OE1
  & Construcción del panel activo~$\times$~día, agregación cuadrante~$\times$~semana y trazabilidad entre sistemas. \\
\addlinespace
Etapa~3 -- EDA
  (Sección~\ref{sec:metodología-etapa3})
  & OE2
  & Diagnóstico de faltantes y extremos; patrones temporales, espaciales y estacionales; correlación y segmentación exploratoria. \\
\addlinespace
Etapa~4 -- Ingeniería de variables
  (Sección~\ref{sec:metodología-etapa4})
  & OE2
  & Definición de respuesta y predictoras (incl.~el rol o reelaboración de la severidad), depuración, transformaciones y partición temporal de entrenamiento y prueba. \\
\addlinespace
Etapa~5 -- Modelado predictivo
  (Sección~\ref{sec:metodología-etapa5})
  & OE2, OE3
  & Entrenamiento, validación y comparación de modelos; análisis de importancia de variables. \\
\addlinespace
Etapa~6 -- Cierre y documentación
  (Sección~\ref{sec:metodología-etapa6})
  & OE4
  & Documentación reproducible, reportes técnicos y artefactos orientados al uso operativo y regulatorio. \\
\bottomrule
\end{tabular}
\end{table}

\noindent En síntesis, las Etapas~1 y~2 materializan el OE1; las Etapas~3 y~4 sustentan el OE2; la Etapa~5 concentra el OE3 (con apoyo interpretativo al OE2); y la Etapa~6 cierra el ciclo con el OE4.
