...
\begin{enumerate}
    ...
    \item \textbf{Variables respuesta:} $Y = \texttt{n\_eventos}$ (proxy de frecuencia, asociada conceptualmente a SAIFI) y, de forma complementaria, $Y' = \texttt{duracion\_total}$ (proxy de duración acumulada, asociada a SAIDI). Los indicadores regulatorios SAIDI/SAIFI solo se emplean como $Y$ si se reconstruyen con la fórmula CREG; en este trabajo se modelan los proxies anteriores.
    \item \textbf{Impacto posterior} (descriptivo / severidad): \texttt{usuarios\_afectados}, \texttt{elementos\_afectados} y, en general, métricas que solo se conocen al ocurrir o cerrar el evento. Sirven para describir severidad en el EDA y para perfiles territoriales; \textbf{no} se emplean como predictoras contemporáneas de $Y$ o $Y'$ en la misma semana~$t$. Análogamente, $\texttt{duracion\_total}_t$ es respuesta cuando se pronostica continuidad, pero constituye fuga si se incluye como covariable al modelar $\texttt{n\_eventos}_t$.
\end{enumerate}
