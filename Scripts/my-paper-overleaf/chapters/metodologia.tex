...
Los resultados de los seis modelos estimados (cuatro de panel semanal y dos espaciales formales, para SAIDI y SAIFI) se organizaron en tablas homologadas de desempeño comparado, acompañadas de figuras de valores observados frente a predichos y de importancia de variables, presentadas en el Capítulo de Resultados (Sección~\ref{sec:resultados-etapa5}).

\subsection{Métricas de desempeño}
Sobre el conjunto de prueba se calcularon RMSE, MAE y $R^2$ en escala $\log(1+y)$ para los cuatro modelos de panel, y AIC para los modelos espaciales SAR/SEM, siguiendo las definiciones formalizadas en la Sección~\ref{sec:metodos-modelos}.

\subsection{Hallazgos principales}
Los hallazgos de las seis etapas se consolidaron en una síntesis única que articula el resultado principal de cada etapa con los tres objetivos específicos del proyecto (Sección~\ref{sec:resultados-etapa6}).

\subsection{Interpretación técnica y operativa}
Los resultados se tradujeron en implicaciones para la gestión operativa del operador de red, priorizando variables de infraestructura y exposición territorial sobre las cuales existe margen de acción (densidad de red, antigüedad de activos, composición de estrato). Esta interpretación se desarrolla en el Capítulo~\ref{sec:discusion}.

\subsection{Generación de reportes}
Cada etapa del proyecto se documentó en informes reproducibles (R Markdown renderizados a HTML), consolidados en el Apéndice~\ref{app:informes-html}, junto con el código fuente de extracción, integración y modelado (Apéndice~\ref{app:codigo}).

\subsection{Discusión de resultados}
El análisis crítico de fortalezas, limitaciones y comparaciones entre los modelos estimados se desarrolla en el Capítulo~\ref{sec:discusion}, para no duplicar contenido entre los capítulos de Metodología y Discusión.

\subsection{Limitaciones y trabajos futuros}
Las limitaciones identificadas a lo largo del proyecto (entre ellas, el sesgo de cobertura de emparejamiento 2021--2023 y la señal climática contemporánea débil) y las líneas de trabajo futuro se consolidan en la Sección~\ref{sec:trabajo-futuro}.
