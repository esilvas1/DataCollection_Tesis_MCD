...
\begin{enumerate}
    ...
    $\mathrm{SAIFI}=\texttt{usuarios\_afectados}/UT$ y
    $\mathrm{SAIDI}=(\texttt{duracion\_total}\times\texttt{usuarios\_afectados})/UT$,
    con $UT=650{.}000$ usuarios totales del operador (parámetro fijo, actualizable). \texttt{n\_eventos} y \texttt{duracion\_total} son insumos de esa construcción y del historial \emph{ex ante}, no las respuestas a predecir.
