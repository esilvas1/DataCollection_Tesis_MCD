Para el desarrollo del modelo de pronóstico de $SAIFI$ y $SAIDI$, se utilizará una base de datos histórica de eventos de falla reportados en el sistema de distribución. La tabla principal consolidada tiene $N = 87{.}007{.}074$ observaciones ($A = 47{.}649$ activos eléctricos únicos $\times$ $T = 1{.}826$ días), correspondientes al panel \textbf{activo $\times$ día} entre el $1$ de enero de $2021$ y el $31$ de diciembre de $2025$.
...
\begin{itemize}
    \item \textbf{Duración} (\texttt{DURACION\_min}, en minutos): minutos de interrupción del activo en el día de observación, prorrateados según la intersección de \texttt{[Fecha\_Desconexion, Fecha\_Conexion]} con ese día calendario. Si el evento cruza varios días, su duración se reparte entre ellos; si hay varios eventos el mismo día, se suman. No se asigna la duración total del evento solo al día de desconexión. En días sin falla, el valor es $0$.
    
    \item \textbf{Frecuencia} (\texttt{FRECUENCIA}): número de interrupciones cuyo inicio (\texttt{Fecha\_Desconexion}) ocurre en el día (sin prorrateo de la frecuencia).
\end{itemize}
