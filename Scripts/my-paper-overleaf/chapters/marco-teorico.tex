...
\begin{enumerate}
    ...
    \item \textbf{Impacto posterior} (descriptivo / severidad): \texttt{usuarios\_afectados} y \texttt{elementos\_afectados} solo se conocen al ocurrir o cerrar el evento. \texttt{usuarios\_afectados} entra en el numerador de $Y$ y $Y'$, pero \textbf{no} se emplea como predictora contemporánea. \texttt{elementos\_afectados} queda restringido a la descripción de severidad. Análogamente, $\texttt{duracion\_total}_t$ y $\texttt{n\_eventos}_t$ contemporáneos no entran como covariables al pronosticar $\mathrm{SAIDI}_t$ o $\mathrm{SAIFI}_t$.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Dispersión relativa (coeficiente de variación):}
